...
默认报告目录为 `<session_id>/report`，论文图目录为 `2026-EgoAnchor-Typst/figs/rq1`。论文图从 `static_observation` 中选择首个 *Full* 已锁定、两变体与平台参考均有效的连续 180 个 Unity 渲染帧；该规则不读取误差大小。位置与旋转分别输出三轴帧时间线，遮挡恢复只保留总体指标，不单独绘图。
