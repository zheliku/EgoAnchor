...
class PoseResultFactory:
    ...
    def build(self, observation: PoseObservation) -> anchor_pb2.PoseResult:
        ...
        result.score_consistency = float(observation.score_consistency)
        ...
        result.track_consistency = float(observation.track_consistency)
        result.consistency_mask_iou = float(observation.consistency_mask_iou)
        result.consistency_depth_inlier = float(observation.consistency_depth_inlier)
        result.consistency_depth_alignment = float(observation.consistency_depth_alignment)
        result.consistency_render_visible_ratio = float(observation.consistency_render_visible_ratio)
        result.consistency_observed_visible_ratio = float(observation.consistency_observed_visible_ratio)
        result.consistency_depth_residual_m = float(observation.consistency_depth_residual_m)
        result.consistency_render_area_px = int(observation.consistency_render_area_px)
        result.consistency_expected = bool(observation.consistency_expected)
        result.consistency_status = str(observation.consistency_status or "")
