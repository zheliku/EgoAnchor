...
每个 session 只完成一个 task。通过单 session QC 后，先复制到
experiments/_staging/experiment_1_2/batch_YYYYMMDD_<config-hash>/raw/，并把外层副本命名为
固定 task 名；内部 session_id 和固定文件内容不变。
