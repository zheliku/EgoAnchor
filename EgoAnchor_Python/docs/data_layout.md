...
或重采时创建新的 `vN` 目录。`data exp1-2 stage` 用文件路径和行尾归一后的内容摘要识别原地变化：
修改时间和纯 CRLF/LF 行尾差异都不算改写，因此复制、同步或还原归档不会误报，实质内容变化仍会被
捕获；显式 `validate exp1-2` 用于需要完整深查的情况。
...
`pixi run eval analyze exp1-2` 只写当前活动组合的 `analysis/`，不会修改论文目录。确认结果后，`pixi run eval copy-assets exp1-2`
