...
分析参数契约为 v5，图产物契约为 v9。当前不兼容旧目录、旧文件名或旧 v4--v8 构建清单；产物路径固定。
...
正文候选是一张双栏宽的单排四分区复合图 `figure4_exp3_subjective_outcomes`，全图只用一个方法图例。
四个分区依次显示阶段锚点行为、整体锚定判断、AQ 与 S-TIAS、TiA，共用十二个等宽物理槽。
前三组保留 1--7 原始分，TiA 在自身面板右侧保留独立的 1--5 刻度，量尺名称写在面板标题中；不同量尺不归一化。横轴、纵轴和刻度使用与实验一/二组合图一致的 7.4 pt 基础字号，固定画布中的轴框范围为 4.5%--98.0%，四个分区之间保留 3.5% 画布宽的间距。纵轴标题按默认轴外布局放置，不能挤入刻度数字。PNG 保留固定画布供审计；论文 PDF 按全部可见内容紧裁，并保留 0.020 in 安全边距。

每项结局并列 One-Euro 和 EgoAnchor。透明箱体表示 IQR，箱内横线表示中位数，圆点表示均值；
浅色小点是参与者级得分，浅灰线连接同一参与者，须线采用 1.5 倍 IQR 规则。显著性括号分别使用
所属冻结家族内 Holm 校正后的 p，并只显示 `*`、`**` 或 `***`。逐物体数据只检查方向，
不分别检验或添加显著性标记。
