...
- `mask` 不是 pose 几何正确性的直接证据。遮挡会让 mask 小，但不一定说明 pose 算错了，所以只做有下限的调制。
- `score_reprojection`、`score_depth`、`score_mask` 仍按原始诊断值写入 HUD、JSONL 和 PoseResult。valid 标志只影响 `quality` 合成，不改变这些可见子分。
...
| reprojection disabled，depth 也无信号 | core 回到 1.0，只由 mask/confidence 调制 |
