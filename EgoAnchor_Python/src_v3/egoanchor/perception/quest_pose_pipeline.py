"""Quest stereo + YOLOE + FFS + FoundationPose 的 v3 pose pipeline。
...
class QuestPosePipeline:
    """v3 Quest object pose estimation pipeline。"""
    ...
    def __init__(
        ...
        mask_snapshot_window: str = "EgoAnchor v3 mask",
    ) -> None:
    ...
    def reset_tracking_state(self) -> None:
        ...
        if self.cutie is not None:
            self.cutie.reset()
        logging.info("v3 pose pipeline tracking state reset")
    ...
    def _refresh_calibration(self, msg: quest_pb2.QuestCameraInfo) -> None:
        ...
        if self.cutie is not None:
            self.cutie.reset()
        logging.info("v3 calibration updated: calib=%dx%d baseline=%.4fm fx=%.1f", calibration.calib_width, calibration.calib_height, calibration.baseline_m, self.cam_k[0, 0])
    ...
    def _log_stats(self, diagnostics: FrameDiagnostics, observation: PoseObservation) -> None:
        ...
        if self.log_stats_interval <= 0 or self._processed_count % self.log_stats_interval != 0:
            return
        logging.info(
            "v3 pose frame=%s phase=%s has_pose=%s det=%d depth(mask)=%.3f mask=%.3f score=%.2f fps=%.1f total=%.1fms",
