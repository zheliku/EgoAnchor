...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="EgoAnchor v3 YOLOE-26 实时掩码验证工具")
...
def run_yoloe_mask_probe(config_path: str | None = None, args: argparse.Namespace | None = None) -> None:
    ...
    window_name = "EgoAnchor v3 YOLOE Mask Probe"
    ...
    try:
        ...
        logging.info("[YOLOEMaskProbe:v3] listening on %s. Keys: s save, q/ESC quit.", receiver.endpoint)
        ...
        while True:
            ...
            if stereo_msg is None:
                ...
                if now - last_wait_log_time >= float(pose_cfg.wait_log_interval_s):
                    logging.info(
                        "[YOLOEMaskProbe:v3] waiting received=%d stereo=%d camera_info=%d decode_failed=%d",
                        stats.received,
            ...
            if displayed_frames % 30 == 0:
                ...
                logging.info(
                    "[YOLOEMaskProbe:v3] frame=%s fps=%.1f det=%d selected=%d mask=%.3f infer=%.1fms",
                    decoded.frame_id,
                    fps,
                    result.det_count,
