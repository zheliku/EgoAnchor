...
- 接收 Unity v3 发送的 multipart `[topic_utf8, protobuf_bytes]`；
...
def run_quest_video_probe(config_path: str | None = None) -> None:
    ...
    waiting_image = make_waiting_image(
        ...
        title="Waiting for EgoAnchor v3 Quest stereo frames...",
    )
    displayed_frames = 0
    last_stats_frame = 0
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="EgoAnchor v3 Quest -> Python 实时图像通信 demo")
