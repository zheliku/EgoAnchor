...
**同步状态（2026-07-26）**：`egoanchor_cn_ai_v8.tex` §5.5 已按 v5.1 核对修正版重写（13 条目枚举与官方构念用词、78 评分、24 平衡单元、方法级问卷移到全六区块后、AQ/TiA/S-TIAS 的 adapted 署名与当前样本信度表述、Wilcoxon 主分析、最终六项测量、Gottsacker 2024 有界表述），XeLaTeX 编译通过、无未定义引用。**唯一正式采集与分析工作簿是 `2026-EgoAnchor/material/EgoAnchor_Experiment3_DataCollection_24P_v5_1_Beautified_Checked_VSCodeSafe.xlsx`**（精简 6 表：README / Verification_Audit / Questionnaire 问卷一表 / Participants / Records 记录一表三段堆叠 / Analysis 回填壳；结构由 v5.1 生成器产出、经用户美化复核定稿，**生成脚本已随定稿移除**，此后内容修改直接在该文件进行并在 `Verification_Audit` 补记录；无公式/数据校验/冻结窗格，节标题行含合并单元格——程序读取须按值读取；原始评分不得被反向分覆盖）。**参与者问卷包**：`material/EgoAnchor_Experiment3_Complete_Questionnaire_v5_1_Bilingual.md` 为唯一事实源，同名 docx 由 `material/build_exp3_questionnaire_docx.py` 从 md 确定性生成——改问卷先改 md 再重跑脚本，不得手改 docx；问卷包已按 v5.1 修正（Q2 锐化措辞、最终 F1–F7 含偏好强度与区分信心、TiA 中性序号按官方相对顺序 1,2,6,7,10,11,13,15,16,19 交错呈现并含"无法回答"选项、区块/方法级指导语与蓝本定位：默认头显内头锁 UI 施测，纸质仅作设备故障应急）。历史模板（v3、v4、GPT 30 题方案、GPT 25 表初版、素版 Verified）均在 `material/old/`，问卷调研语料在 `material/old2/`，**一律不得用于正式采集**。
...
所有 AI 或模型生成的实验三数据只用于采集表和分析流程演练，必须在文件名、工作簿首屏和报告中明确标为合成，并与正式参与者数据及分析入口隔离；不得进入论文结果、效应量、功效或样本量推断，也不得与真实数据合并。面向 VS Code Office Viewer 的演练工作簿使用与现有 `VSCodeSafe` 模板一致的普通单元格安全子集，不依赖 Excel Table、条件格式、数据验证、合并标题、自动筛选或冻结窗格；统计约束由生成与验证代码执行。**Round 2 起演练工作簿按用户要求改用公式实时计算问卷小分**（这是对「无公式」约定的有意偏离，仅限演练工作簿；正式采集表 `..._Beautified_Checked_VSCodeSafe.xlsx` 仍保持无公式）：`Records` 是唯一输入且只放原始评分（TiA 反向项存原始分，反向计分只在派生层做），`Derived` 四级派生与 `Analysis` 的 Mdn/IQR、配对差、dz、逐物体均值、TOST 两侧均值与各项计数全为公式，只有 Wilcoxon 的 W/p、Holm 校正 p、r_rb 自举 CI 与信度 α/ω 由 Python 写数值。**公式不能只写不算**：openpyxl 与 Office Viewer 都不计算公式，须由 `sim_r2_formula_eval.py`（受限求值器，只支持实际用到的 13 个函数、遇未支持语法即抛错）求值后把 `<v>` 注入 sheet XML，并置 `fullCalcOnLoad="1"` 使 Excel 打开时仍全量重算。注入时**必须同时删除成对 `<v>..</v>` 与自闭合 `<v />`**——openpyxl 为公式单元格写出的正是自闭合空值，只删成对标签会使缓存值恒为空。正确性由双路交叉验证保证：公式文本→求值器→缓存值，对比 `Records` 原始评分→`sim_r2_recalc_verify.py` 手写重算，逐格容差 1e-9。**不要依赖本机 Excel COM 重算**：仓库在网络盘 `P:\` 上，Excel 以受保护视图打开，COM 调用被 `RPC_E_CALL_REJECTED` 持续拒绝，且失败会残留 Excel 进程并锁住输出文件。
