...
**顺序平衡已统一为 24 平衡单元**：3 物体的 6 种全排列 × 互补方法序列 S1(A--B, B--A, A--B)/S2(取反) × A/B 标签到方法的映射，N=24 时每单元 1 人、先行方法 12/12；匿名标签在参与者内全程稳定绑定同一方法，否则最终强制选择无从解释。N=18 下限时优先保先行方法与标签映射平衡。旧的"3 拉丁方 × 2 条件顺序 = 6 组"方案已废弃。**统计已统一为 Wilcoxon 为主**：每条目/子量表分先在三物体上**取均值**（非中位数——三点中位数大量结平损失效能）再逐项 Wilcoxon + 分层 Holm（主证实家族 7 项 Q1/Q8/Q2/Q9/Q3/Q6/Q7，N=24 最严 MDE dz=0.801；已发表量表家族 5 检验 AQ-EQ/AQ-IQ/TiA 两分量表/S-TIAS，TiA 反向项先按 6−raw 换向；Q10 次级；选择、偏好强度、区分信心与开放题描述性），次级用 CLMM（序数；固定效应 = 方法 + 物体 + 方法 x 物体 + 区块位置 + 对象内先后，随机效应 = 参与者截距），方法 x 物体交互只作探索性报告，不使用 ART/ART-C；家族划分采集前冻结。曾出现在 tex 的"CLMM 为主 + ART 备选"表述系与冻结决定冲突的误改，已于 2026-07-26 修复，不得再改回。

**同步状态（2026-07-29）**：`egoanchor_cn_ai_v8.tex` §5.5 已按 v5.1 核对修正版重写（13 条目枚举与官方构念用词、78 评分、24 平衡单元、方法级问卷移到全六区块后、AQ/TiA/S-TIAS 的 adapted 署名与当前样本信度表述、Wilcoxon 主分析、最终六项测量、Gottsacker 2024 有界表述），XeLaTeX 编译通过、无未定义引用。`2026-EgoAnchor/material/reference/EgoAnchor_Experiment3_DataCollection_24P_v5_1_Beautified_Checked_VSCodeSafe.xlsx` 保留为经用户美化复核的只读来源模板；**正式采集和分析的唯一默认输入是 `2026-EgoAnchor/material/EgoAnchor_Experiment3_RawData_24P_v5_1.xlsx`**。新文件有 README / Verification_Audit / Questionnaire / Participants / Records / Derived / Analysis 七表，`Participants` 与 `Records` 只保存人工原始值，TiA 反向计分只在 `Derived` 执行。绿色 `Derived`/`Analysis` 公式实时显示完整性、三物体均值、配对差、Mdn/IQR、SD 与 dz；`Analysis` 固定保留 C1（三物体均值后的主证实统计），并增加 C2（7 个主条目 × 3 个物体的完整配对描述），用于逐物体方向核查和论文分面图。C2 不写 W、p、Holm、显著性结论或 `r_rb`；逐物体推断只按冻结的次级 CLMM 方案执行。为兼容 WPS，实时四分位公式固定使用与 `QUARTILE.INC` 同算法的 `QUARTILE`，不得改回 `QUARTILE.INC`。Wilcoxon、Holm、效应量区间、信度、CLMM 和 TOST 只由 Python 结果工作簿提供。**Python 结果工作簿固定 13 张表**，顺序与其 `README` 内置的工作表索引严格一致并在写入后逐项回读校验，同一个数字只出现在一张表里：`README`、`Sample`（样本流 + 人口学 + 经验 + 安全 + `Design_Balance` 平衡审计，按 `Section` 分节）、`Participant_Audit`、`Results`（唯一结果表，`Family` 列区分主证实 / 已发表量表 / 探索性三族）、`Results_By_Object`（逐对象结果，`p_Holm_Panel` 只在结局内三对象间校正）、`Reliability`、`Model_CLMM`（`Row_Kind` 区分固定效应与逐对象对比）、`Manipulation`、`Choices`（含堆叠为行的偏好×信任交叉）、`Open_Coding`、`Scores_Block`、`Scores_Method`、`Scores_Paired`。确证结论只看 `Results` 的 `p_Holm`；探索性行的 `p_Holm` 与 `Significant` 一律留空，未校正 p 不得被读成结论。`Results_By_Object` 属探索性方向一致性检查，只在同一结局的三个对象间校正、未跨结局校正，只用于论文图的面板内星号。**两处必须按标注解读**：① `r_rb_CI_Status` 标为 `degenerate_at_bound` 时（某结局全部非零配对差同号，`r_rb` 取到 ±1，任何参与者级重采样都得到同一值），百分位区间必然退化为零宽度，论文只能叙述"方向完全一致"，**不得把 `[1.00, 1.00]` 写成置信区间**；当前正式数据中只有 TiA-R/C 属此情形。② `Reliability.Measurement_Unit` 区分两种单位：AQ-EQ/AQ-IQ 的条目分先在三物体取均值，其 α/ω 是**分析单位（三物体均值合成分）的信度**、系统性高于条目级值（`block_mean`）；TiA 与 S-TIAS 每种方法只施测一次，才是常规条目级内部一致性（`method_single`）。两者不可互比，也不可与原量表发表 α 直接对标。**`Model_CLMM` 的标准误当前不可用（已知缺陷，2026-08-01 发现，待修）**：`_fit_clmm` 把 L-BFGS-B 的 `result.hess_inv` 当协方差，而它只是拟牛顿迭代累积的近似逆 Hessian、不是最优点观测信息矩阵。三条独立证据确认其放大 SE：真实数据上与数值 Hessian 的 SE 比值 1.57–2.79（中位 1.78）；已知真值模拟（真实效应 1.20 log-odds、N=24）中经验 SD 0.240 对 L-BFGS 平均 SE 0.914（约 3.8×）；另一批经验 SD 0.164 对 0.904（约 5.5×），Wald 95% 区间实际覆盖率 100%。后果是 CLMM 对七个主条目报告 **0/7** 显著方法主效应，而主分析 Wilcoxon 报告 **5/7**，Q1/Q8/Q3/Q6/Q7 五项结论直接矛盾（优势比 3.0–4.9 合理，但 SE 过大使每个 CI 都跨 1）。**修好前 `Model_CLMM` 的 SE、z、p、CI 一律不得写入论文**；点估计与交互 LRT（基于对数似然差、不依赖 Hessian）不受影响。数值 Hessian 在本似然面近奇异（正定率 21%、条件数约 1e31，步长 1e-4…1e-1 均无改善），不能简单改成数值微分；可选修复是参与者级自举或逐效应似然比 profile 区间（LRT 已验证可用），属统计方法变更、须先经研究者批准。工作簿用 core properties `EgoAnchor.Experiment3.RawData.v5.1` / `formal-participant-data` 标识正式输入，启用数据校验与全量重算，`Derived`/`Analysis` 受保护，且已修复旧 `Analysis!A78:S78` 合并缺陷；模板构造器位于 `egoanchor.eval.experiments.experiment_3`，必须显式指定尚不存在的目标，拒绝覆盖已有工作簿。原始评分不得被反向分或公式缓存覆盖。**参与者问卷包**：`material/EgoAnchor_Experiment3_Complete_Questionnaire_v5_1_Bilingual.md` 为唯一事实源，同名 docx 由 `material/build_exp3_questionnaire_docx.py` 从 md 确定性生成——改问卷先改 md 再重跑脚本，不得手改 docx；问卷包已按 v5.1 修正（Q2 锐化措辞、最终 F1–F7 含偏好强度与区分信心、TiA 中性序号按官方相对顺序 1,2,6,7,10,11,13,15,16,19 交错呈现并含"无法回答"选项、区块/方法级指导语与蓝本定位：默认头显内头锁 UI 施测，纸质仅作设备故障应急）。历史模板（v3、v4、GPT 30 题方案、GPT 25 表初版、素版 Verified）均在 `material/old/`，问卷调研语料在 `material/old2/`，**一律不得用于正式采集**。
...
- `analyze` 只在各自活动分析目录生成结果，不回填主稿；每次构建开始即把统一 `build_result.json` 置为 `building`，只有输入、配置、实现与全部产物摘要冻结后才提交 `complete`。`copy-assets` 只接受 `complete + formal`，先让目标实验构造只读资源计划，再联合校验全部来源、配置/实现/输入/产物 SHA、后缀与目标冲突，全部通过后暂存并以可回滚事务复制。`copy-assets all` 要求两个实验都就绪，不得静默跳过；无参数时默认实验一/二，也可显式指定 `exp3` 或 `all`。实验一/二表格默认复制为 `tables/exp1_static.tex`、`tables/exp1_dynamic.tex` 和 `tables/exp2_design.tex`；正式实验三复制 `figures/panels/figure4_exp3_paired.{png,pdf}`、`figure5_exp3_scales.{png,pdf}` 与 `tables/exp3_subjective.tex`，模拟来源不得复制。图环境 TeX 仍由研究者审阅后手工纳入。主稿编译不属于 `pixi run eval`，当前工作稿编译产物为 `2026-EgoAnchor/pdf/egoanchor_cn_ai_v8.pdf`，仅供内部审阅，不是稳定交付版本。
