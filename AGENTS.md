...
**同步状态（2026-07-28）**：`egoanchor_cn_ai_v8.tex` §5.5 已按 v5.1 核对修正版重写（13 条目枚举与官方构念用词、78 评分、24 平衡单元、方法级问卷移到全六区块后、AQ/TiA/S-TIAS 的 adapted 署名与当前样本信度表述、Wilcoxon 主分析、最终六项测量、Gottsacker 2024 有界表述），XeLaTeX 编译通过、无未定义引用。`2026-EgoAnchor/material/EgoAnchor_Experiment3_DataCollection_24P_v5_1_Beautified_Checked_VSCodeSafe.xlsx` 保留为经用户美化复核的只读来源模板；**正式采集和分析的唯一默认输入已改为 `2026-EgoAnchor/material/EgoAnchor_Experiment3_RawData_24P_v5_1.xlsx`**。新文件有 README / Verification_Audit / Questionnaire / Participants / Records / Derived / Analysis 七表，`Participants` 与 `Records` 只保存人工原始值，TiA 反向计分只在 `Derived` 执行，绿色 `Derived`/`Analysis` 公式实时显示完整性、三物体均值、配对差、Mdn/IQR、SD 与 dz；Wilcoxon、Holm、效应量区间、信度、CLMM 和 TOST 只由 Python 结果工作簿提供。工作簿用 core properties `EgoAnchor.Experiment3.RawData.v5.1` / `formal-participant-data` 标识正式输入，启用数据校验与全量重算，已修复旧 `Analysis!A78:S78` 合并缺陷；模板构造器位于 `egoanchor.eval.paper_analysis.experiment_3`，必须显式指定尚不存在的目标，拒绝覆盖已有工作簿。原始评分不得被反向分或公式缓存覆盖。**参与者问卷包**：`material/EgoAnchor_Experiment3_Complete_Questionnaire_v5_1_Bilingual.md` 为唯一事实源，同名 docx 由 `material/build_exp3_questionnaire_docx.py` 从 md 确定性生成——改问卷先改 md 再重跑脚本，不得手改 docx；问卷包已按 v5.1 修正（Q2 锐化措辞、最终 F1–F7 含偏好强度与区分信心、TiA 中性序号按官方相对顺序 1,2,6,7,10,11,13,15,16,19 交错呈现并含"无法回答"选项、区块/方法级指导语与蓝本定位：默认头显内头锁 UI 施测，纸质仅作设备故障应急）。历史模板（v3、v4、GPT 30 题方案、GPT 25 表初版、素版 Verified）均在 `material/old/`，问卷调研语料在 `material/old2/`，**一律不得用于正式采集**。
...
- `egoanchor.eval` 包级入口只导出 schema-v2、QC 和 Stage 1 workbook 基础设施；论文分析必须从 `egoanchor.eval.paper_analysis.experiment_1_2`、`egoanchor.eval.paper_analysis.experiment_3` 的包级入口或离线 CLI 显式进入，运行时服务不得因论文绘图依赖加载失败。
