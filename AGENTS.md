...
- **附加材料**：英文投稿版本的唯一入口是 `2026-EgoAnchor/egoanchor_en_supplementary.tex`，以 `tables/en/exp3_subjective_supp.tex` 提供 3 页独立 PDF。它只保留英文主稿明确承诺的五项：完整运行时参数、7 个自定义锚定题项、标准化量表的构成/量尺/施测与计分、12 项完整主观统计、量表信度；不复述标准化量表原题、不附原始受试者数据，也不新增数据公开承诺。参数旧源 `supplementary_runtime_parameters.tex` 只作转录依据。正文内联的取值与附加材料必须同步；正文不再出现「附录」字样，且代码只承诺接收后发布。
...
- 调试窗口与录制：`diagnostics/debug_view.py` 将 pose 拆为 left/right/mask/depth/pose 五张 640x480 子图；VCD 的四张基础图为 640x480，两张残差对比图保留 640x480 原图并在右侧追加 88 像素色标。Pose 与 VCD 窗口始终共用刷新条件并从同一 `pipeline_output` 生成，不区分是否录制；`tracking_server.py` 按 `V` 切换 12 路 10 FPS 异步 H.264 MP4 录制（Windows 使用 MSMF，其他平台不可用时回退 `mp4v`），每次在 `EgoAnchor_Python/data/debug/videos/` 下创建独立时间戳目录，mask 无数据时写黑帧，所有视频逐帧同源且不生成 PNG。
