...
- `egoanchor.eval.contracts` 的 workbook 契约当前为 breaking v2，完整保留 Stage 2 所需的对齐原始位姿、时间、reference、render 和事件字段；pose 数组按标量列写出，显式声明主外键。CSV 契约当前为 breaking v2，指标契约为 breaking v3，参数契约为 breaking v2；参数入口固定为 `egoanchor/eval/config/analysis_params.toml`。
- `analysis_params.toml` 是唯一科学参数入口，Stage 2 使用该文件原始字节的 SHA-256 作为参数集标识。参数 v2 固定 linear 分位数、二阶 1 Hz Butterworth 零相位 HP-RMS、首尾 1 秒 drift、参考运动检测、visible response、settling、0--500 ms lag、相邻 tick jump 和 target-visible recovery；所有 display/reference 指标必须显式使用 pose 有效掩码，lag 和持续条件不得跨大时间间隙。指标 v3 补齐实验一旋转、峰值、unlock/relock、lag residual、恢复更新数和 fresh output 指标；场景汇总同时记录尝试数、成功样本数和成功率，失败事件不得被静默丢弃。
...
- schema-v2 reader 按 dataclass 契约严格检查固定字段和跨表稳定键，并把 `python_session.json` 的停止态 writer 统计、Python host/version 合并到内存 manifest；只有 `python_stopped` 且 `python_events.jsonl`、`unity_events.jsonl` 的实际行数分别匹配两端冻结统计后，才原子发布可重建的 `events.jsonl`。半同步、pending、错配或非法 fragment 不得留下部分派生文件，也不得进入正式分析；Mutagen 完成同步后允许对同一目录直接重试。
