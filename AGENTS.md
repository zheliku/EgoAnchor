...
- **实验三：日常物体上的跨对象感知评价**。`2 条件 x 3 物体 = 6 区块` 被试内设计，每区块 3 任务 + 7 条目，只比较 *One-Euro Anchor* 与完整 *EgoAnchor*；**2026-07-25 起改为纯主观评价，不采集任何客观任务数据**。当前只冻结设计，实验一/二完成后再启动正式采集。完整方案见 `2026-EgoAnchor/experiment_3_design_zh.md`。
...
实验三的完整冻结设计固定在 `2026-EgoAnchor/experiment_3_design_zh.md`，`plan.md` 只保留摘要、论文 §5.5 只保留可发表叙述；改动前必须先读该文件，其开头的"本轮决策记录"表列出了 2026-07-25 被推翻的旧决定，**不得改回**。要点：**实验三是纯主观评价，不采集任何客观任务数据**（无任务时间、无成功率、无行为探针；T3 的四按钮面板行为任务已删除）。条件只有 *One-Euro Anchor* 与完整 *EgoAnchor*，*Arrival-Hold* 只作训练阶段演示、不进入推断统计。交叉核心对象为 `blue_mouse`、`stapler` 与 `gamepad`（覆盖尺寸/纹理/几何三个维度；手柄强制单手握持单侧搬移，使另一手保持握持控制器），`earphone` 作困难样例只由操作员采集并兼作训练物体。**区块结构是物体最外层、两条件嵌套在同一物体内**：物体外层因为 `--object` 只在服务启动时读取、协议无运行时切换动作，且同一物体的两条件由同一次感知会话服务；条件内层是灵敏度选择，同物体相邻 A/B 给出最紧配对。三项任务为静止观察、拿起放下、遮挡恢复，**固定顺序**（对应三个不同运行时状态，非同一构念的三次测量），合计 45--60 s 后统一评分。两类人为探针（读 4 字符码、射线选中小目标）均已评估否决：整体游移不影响可读性、单次注视内抖动仅 0.17 个字高，正确率会撞天花板；小目标选择受追踪补偿影响；两者还要额外花版面解释。**遮挡时长必须使锚点停留在 `FrozenUncertain` 而不进入 `Lost`（0.6--0.9 s 起，预实验校准后冻结）**：`AnchorStateMachine.cs:105-115` 的实际分支是 `<=0.45 s` 滑行、`0.45--1.0 s` 冻结、`>=1.0 s` 丢失，因此 **0.6--0.9 s 落在冻结区间而非滑行区间**（旧文档称"coasting 区间"与 0.45 s 滑行上限自相矛盾，已于 2026-07-25 修正）。更长的遮挡（含旧文档的 2 s 与 2.5 s）会使两条件都进入 `Lost` 并等待同一次服务器 REGISTER（中位数 750.26 ms），使恢复条目主要反映感知后端而非运行时；该区间还须避开 0.45 s 边界，否则计时抖动会把试次随机分到两种生命周期状态。生命周期状态分布与两条件的候选/VCD/接纳率一致性是必须报告的操纵检验。运行时参数不得为实验三修改。**测量为单一层级**：每个条件--物体区块后填 7 个条目（Q1 静止稳定/Q2 运动附着/Q3 恢复一致，七点同意度；Q4 合理可信/Q5 同步性，七点强度、单独成页；Q6 依赖意愿/Q7 稳定--响应平衡，七点同意度），全部以该区块为评价对象，共 42 个评分，加最终强制选择与 2 道开放题。逐项分析，**不得合并总分或报 Cronbach's alpha**，正文标注为非标准量表。**不得引入 SUS**（两条件共享完全相同的界面与交互）；**不得引入 NASA-TLX**（两条件任务动作与认知要求完全相同）；**不得以 CRIQ 之名署名 Q4/Q5**——已核查无法确证存在名为 CRIQ 的已验证量表（SelfBlending 归给 Gottsacker et al. ISMAR 2021，但该文标题不含量表名，缩写与分量表分类系其自身表述；该领域联网检索无同名已验证量表，唯一同名 `CRIq` 是无关的 Cognitive Reserve Index），论文只写"改编自既有跨现实交互提问的两个单项"并说明归属链。统计：主分析先把每条目在三物体上**取均值**（非中位数——三点中位数大量结平损失效能）再逐条目 Wilcoxon + Holm，次级用 CLMM（序数；固定效应 = 条件 + 物体 + 区块顺序，随机效应 = 参与者截距），条件 x 物体交互只作探索性报告，不使用 ART/ART-C。样本量 N=24 目标、N=18 下限（平衡组为 6 组，N 必须是 6 的倍数），不得用实验一的片段级中位数推导 n 或断言感知阈值。日常物体上没有同平台参考位姿，实验三不得报告绝对配准误差，只报主观评价和无需真值的自参考稳定性日志，且不得作中介效应主张；**也不得声称提供任务表现证据**，该边界必须在讨论/局限中明说。Exp3 使用独立的 2 runtime `variant_matrix_id`、独立启动门禁和与 schema-v2 隔离的日志与分析模块，不复用实验一/二的九路矩阵。

**实验三的测量结构冲突现已收窄到采集模板一侧。** 工作树中的 `egoanchor_cn_ai_v8.tex` §5.5 已改写为冻结结构（`2 \times 3 \times 7=42` 个评分、单一层级、逐条目分析、不合并总分、不报 Cronbach's alpha、不以 CRIQ 之名署名 Q4/Q5），与 `experiment_3_design_zh.md` 一致；`\input{tables/exp3_subjective.tex}` 已移除，`tables/` 下无该文件。因此旧结构（方法最外层 + 物体级 3 项/方法级 7 项 + CRIQ 复合分 + alpha）现在只残留在采集模板 `2026-EgoAnchor/material/EgoAnchor_Experiment3_DataCollection_24P_VSCodeSafe_v3.xlsx` 中，该模板与论文和设计文档均不兼容，**在按冻结结构重做前不得用于正式采集**；它的 `CRIQ3_Responsiveness`、`Trust1_Position`、`Trust2_After_Recovery` 三项及两个复合分列在冻结设计中已删除，不得回填。

**顺序平衡存在文档分歧，需统一**：`experiment_3_design_zh.md` 第一节写 3 个拉丁方物体序列 x 2 种条件顺序 = 6 个平衡组、条件顺序在参与者内固定；论文 §5.5 写 3 个物体的六种全排列各 4 人、每种物体顺序内再用互补标签序列 S1(A--B, B--A, A--B) 与 S2(取反)，得 12 个平衡单元。论文方案更强（参与者内交替先行方法，消除先行方法与方法身份的共线），且匿名标签 A/B 必须全程稳定绑定到同一方法，否则最终强制选择无从解释。

**实验三问卷已于 2026-07-26 扩为 v4，权威文件是 `2026-EgoAnchor/experiment_3_questionnaire_design_zh.md`**（条目原文、量表锚点、呈现顺序、分析家族的唯一来源；`experiment_3_design_zh.md` 第五节已改为指向该文件的重定向块 + 变更摘要，其下方 v3 条目表仅为历史记录，不得据此实现问卷程序）。要点：**9 个核心条目 + 1 个可选**，每人 `2 x 3 x 9 = 54` 个区块级评分（启用可选 Q10 则 60）。**Q1--Q7 的编号与构念不变**，新增 **Q8 `OBJ_POSITION_CORRECTNESS`（位置正确）**、**Q9 `OBJ_ORIENTATION_CONSISTENCY`（姿态一致）**、**Q10 `OBJ_POST_PLACEMENT_SETTLING`（放置后稳定，默认不启用）**。Q8 是必需的：Q1 只问"保持稳定"，冻结在错误位姿的锚点会拿满分，而静止指标本身要求同时报绝对误差以避免该虚假优势；旧 `TRUST_POSITION` 的"与 Q1 重复"删除理由不成立（稳定与正确是正交维度），改以措辞差异 + 呈现顺序隔三页处理响应定式。Q9 是必需的：旋转证据必须独立报告、不得用平移收益替代。**Q1 措辞已改为"虚拟内容保持在同一位置不动"**（删去"在真实物体上"），使 Q1 只承载时间稳定性、Q8 只承载空间正确性。**统计改为分层 Holm**：主证实家族 7 项（Q1/Q8/Q2/Q9/Q3/Q6/Q7）、次级家族 Q4/Q5/Q10、TiA 按原量表单独计分、其余描述性不作推断；主家族仍 7 项，故 N=24 最严 Holm 的 MDE 仍为 dz=0.801。家族划分必须在采集前冻结。**首次引入已验证工具 TiA**（Körber 2019，`koerber2019tia`，doi:10.1007/978-3-319-96074-6_2，已加入 bib），只取 Reliability/Competence 与 Understanding/Predictability 两个分量表，在每个条件的三个物体区块全部结束后施测一次；**该量表的分量表名称、条目数与条目原文尚未逐字核对原文**（撰写时 github/PMC/Frontiers/量表库正文抓取均被网络策略阻断），采集表 `TiA_Items` 表留空待研究者核对后填写，**核对完成前不得把任何 TiA 条目措辞写入论文或问卷程序**，bib 条目的 volume/pages 同样待核实或直接删去只留 DOI。禁止引入 SUS、完整 NASA-TLX、IPQ/临场感量表、具身量表、UEQ/AttrakDiff；可选的 Paas 式心理努力单项同样未核对、默认不启用。**联网核查结论：不存在测量"虚拟内容在真实物体上的配准质量"的已验证量表**，该结论应写入论文测量说明，它把"使用非标准量表"从缺陷转为该构念尚无现成工具的必然结果。

实验三采集表的当前版本是 `2026-EgoAnchor/material/EgoAnchor_Experiment3_DataCollection_24P_v4.xlsx`，由 `material/build_exp3_collection_template.py` 可复现生成（10 张表、24 人平衡分配已预填、评分单元格带 1--7 整数校验、无任何复合分列）。旧 `EgoAnchor_Experiment3_DataCollection_24P_VSCodeSafe_v3.xlsx` 已作废，不得用于正式采集。**尚未同步**：`egoanchor_cn_ai_v8.tex` §5.5 仍是 7 条目叙述（第 430 行的条件--物体级评分段含 `2 \times 3 \times 7=42`，需改为 9 条目与 54，并补统计段的家族划分与 TiA 施测/署名段）。四面板图的面板选择已在 tex 的 TODO 注释中改为 Q1/Q8/Q3/Q6（不选 Q2：其起停转换代价与中段对齐残差方向相反，预期近零效应）。

`material/` 下的 **AI 模拟演练数据**仅用于设计与分析流程验证，**不得作为真实实验数据、论文证据或与真实数据合并**：`EgoAnchor_Experiment3_Simulated_Claude-Fable-5.xlsx` 按已作废的 v3 旧结构生成；`EgoAnchor_Experiment3_Simulated_Claude-Opus-5.xlsx` 与 `simulate_exp3_claude_opus_5.py`、`EgoAnchor_Experiment3_Simulated_Claude-Opus-5_Report_zh.md` 按冻结结构生成（种子 20260726，效应量锚定实验一/二 v4 实测差异，含 6 排列 x S1/S2 x 标签映射共 24 个平衡单元、TOST 等价操纵检验与 v3 字段沿革表）。

所有 AI 或模型生成的实验三数据只用于采集表和分析流程演练，必须在文件名、工作簿首屏和报告中明确标为合成，并与正式参与者数据及分析入口隔离；不得进入论文结果、效应量、功效或样本量推断，也不得与真实数据合并。面向 VS Code Office Viewer 的演练工作簿使用与现有 `VSCodeSafe` 模板一致的普通单元格安全子集，不依赖 Excel Table、条件格式、数据验证、合并标题、自动筛选或冻结窗格；统计约束由生成与验证代码执行。
...
- `pixi run eval` 提供 `config`、`sessions`、`stage`、`promote`、`qc`、`preprocess`、`analyze`、`copy-assets` 和 `rebuild`。操作路径、PNG/PDF 发布清单和三张表格 TeX 的论文目标路径只从 `egoanchor/eval/config/batch.toml` 读取，不使用 shell 环境变量或路径参数；主稿编译不属于评估 CLI，论文统计参数仍只属于 `paper.toml`。文件系统或工具错误返回 1，批次、schema、QC 或论文输入契约失败返回 2。
...
- `analyze` 只在活动分析目录生成 TeX，不回填主稿；`copy-assets` 按本次 `build_result.json` 和 `batch.toml` 显式发布当前实验面板、指定 replay/relay PNG/PDF 与三张表格 TeX。表格默认发布为 `tables/exp1_static.tex`、`tables/exp1_dynamic.tex` 和 `tables/exp2_design.tex`；图环境 TeX 仍由研究者审阅后手工纳入。主稿编译不属于 `pixi run eval`，当前工作稿编译产物为 `2026-EgoAnchor/pdf/egoanchor_cn_ai_v8.pdf`，仅供内部审阅，不是稳定交付版本。
