...
| `2026-EgoAnchor` | 当前 LaTeX 中文论文主稿 v6、v1-v6 历史版本、VGTC 模板与图像资产 |
| `2026-EgoAnchor-Typst` | 已迁移的 Typst 历史来源与代码事实技术流程文档；不再作为论文写作入口 |
...
- `has_output_pose` 只信任 `PoseToAnchorRuntime.TryGetOutputPose`，决定 runtime availability；`has_display_pose` / `display_pos` / `display_rot` 记录用户实际看到的 Transform，包括 Lost 后 hold-last。RQ2 各系统配置的实时误差、显示更新率与未更新帧对占比使用 display pose，锚点输出覆盖率仍使用 `has_output_pose`。执行顺序固定为 `PoseToAnchorRuntime(-50) → DynamicObjectAnchor(0) → EvalRecorder(50)`。
...
论文源：`2026-EgoAnchor/egoanchor_cn_v6.tex`（当前中文主稿 v6）；v1-v6 均已迁移为同目录下的 LaTeX 版本，`template.tex` 是用户指定的 IEEE VR / VGTC 模板基线，图像资产位于 `2026-EgoAnchor/figs/`。正文、标题、图注、表格和行距继承 `vgtc` 默认设置，中文只补充 XeLaTeX / `xeCJK` 字形支持，不使用自定义字号或整体缩放。参考文献：`2026-EgoAnchor/egoanchor_cn.bib`。代码事实文档仍为 `2026-EgoAnchor-Typst/egoanchor_code_derived_technical_flow.md`；Typst v1-v6 仅保留作迁移审计，不再作为写作入口。`docs/architecture/` 已完全删除。
...
- RQ1 的消融覆盖 `static_observation` 与 `occlusion_recovery` 两个静止场景，不进入 RQ2 动态场景
- RQ1 的 `0.03 m/s` 是离线分析的参考静止判定阈值，正文写作「平台参考判定为静止（参考线速度不超过 0.03 m/s）」；不得称为「静止锚定阈值」，避免与运行时 StaticLock 的进入阈值混淆。
- RQ2 正文统一使用「未更新帧对占比」「锚点输出覆盖率」「最新观测时延」；不用「保持帧比例」「锚定输出有效帧占比」「观测年龄」。
...
```
默认 `report_dir=<session_dir>/report`、`figs_dir=2026-EgoAnchor/figs/rq1`。

**测试**：`src/egoanchor/eval/tests/test_rq1_analyze.py` 覆盖场景过滤；`src/egoanchor/eval/tests/test_rq1_plot.py` 覆盖锁定状态选窗、固定帧数和陈旧图清理；`src/egoanchor/eval/tests/test_jitter.py` 覆盖连续段切分与纯运动拒绝。只支持 `python -m egoanchor.eval.research.rq1.analyze` 包入口，不支持直接执行脚本文件。
...
**连续性、动态精度与最新观测时延**：`display_update_rate_hz`、`display_hold_fraction` 与 `tracking_availability` 在论文中分别称显示更新率、未更新帧对占比与锚点输出覆盖率。前两者只统计同一 trial 内、相邻两端均为 `analysis_motion` 且有 display pose 的帧对，不跨速度超限或参考失效空窗；渲染误差使用 `display_*` 与同刻新鲜平台参考逐帧比较，锚点输出覆盖率只使用 `has_output_pose`。RQ2 主文只把 `observation_age_ms` 作为最新观测时延报告中位数/P95；`smoothing_delay_ms` 及其有限值覆盖率保留为分析报告中的实现诊断，不进入主表或正文。当前 RQ2 不报告经验运动 lag。结果为帧层描述性汇总，不执行回归、运行包络或 bootstrap。既有数据的 `0.8 m/s / 180 deg/s` 上限是在查看参考速度分布后设定的事后描述性范围，论文必须明示；它不用于假设检验或运行边界推断。
...
```bash
cd EgoAnchor_Python
pixi run python -m egoanchor.eval.research.rq2.analyze --session-dir data/eval/<session_id> --figs-dir ../2026-EgoAnchor/figs/rq2 --zoom-frame-count 120
```
