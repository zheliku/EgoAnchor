...
- 面向新采集批次的手动复现步骤固定记录在 `2026-EgoAnchor/experiment_1_2_analysis_reproduction_manual_zh.md`；新批次默认由 `pixi run eval stage --promote` 整批 QC、复制、生成工作簿并原子切换，随后依次 `analyze` 和显式 `copy-assets`。单独 `promote` 只用于复用已有完整暂存批次。`qc` 只作诊断，`preprocess` 只在明确重建当前 Stage 1 工作簿时使用；`rebuild` 用于从当前 raw 一次重建，且不发布图片；论文 PDF 由论文工作流手工编译。
...
- 同一分析批次不得包含重复 `session_id`，且固定 formal run kind、对象、对象模型、协议、整体配置哈希、冻结参数集和 runtime 定义必须一致。`stage` 和 `promote` 已执行全量 workbook-v2 回读；`analyze` 只保留 task 1--5、完成场景、共同身份和运行时矩阵契约检查，避免重复完整工作簿验证。`stage` 接收 `data/eval` 下的目录名，目录可保留 task/v4 人工标签；批次身份只取 manifest 的不可改写 `session_id`，批次目录固定为任务 1--5 session 时间组成的 `batch_<task1-time>_..._<task5-time>`，配置哈希仍由 manifest、工作簿 provenance 和 QC 核对。`stage --promote` 在整批成功后自动切换活动批次，无需手工输入批次名。重复 `stage` 会先完整重建，只有新批次成功后才替换同名暂存批次。没有目标实验完成任务的 session 可随同批次输入，但不参与该实验指标。Mutagen `logs-5090` 启用期间原始目录内容、内部固定文件名和 manifest `session_id` 均不得修改。
