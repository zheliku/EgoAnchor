...
- Patent：`EgoAnchor_Invention_Patent/13148-权利要求书-检索结果/01_版本稿/` 采用 `vNN` 版本化工作流；`active/` 只保留当前主稿和对应生成脚本，不覆写代理原稿或历史版本；当前主稿为 `active/v54_透视混合现实视觉帧锚定_申请主稿.md`；当前阶段仅维护版本化 `md` 主稿，`.docx` 由用户自行维护；专利公式与“其中”说明必须和当前实现一致，`.docx` 公式使用原生 Word/WPS 数学对象；当前主稿中 `score_depth` 的中性值必须写为 `0.5`，`-1` 仅用于 `color_reprojection` 缺失，且深度有效覆盖阈值应与当前实现一致写为 `0.10`；输出策略公式必须贴合 `BlendStrategy` / `DelayedInterpStrategy` / `RawPassthroughStrategy` 的实际实现，不得回写旧的统一残差平滑公式，其中 `raw_passthrough` 应写为输出最近控制点位姿 `P_n`，不得误写为控制点状态 `z_n`；阶段门控、连续高质量帧置信和深度项有效性分支应优先采用纯符号表达，不要回退成自然语言条件；`R_{mask,t}` 的分段表达应与 `pose_quality.py` 中 `_mask_factor(...)` 的真实区间完全一致，不得保留重叠区间或额外常数段；静止锚定部分不得杜撰锁定期“缓释跟随”输出，必须与 `StaticLockController` 的真实行为一致，即锁定期直接输出 `lockedPose`，死区与 CUSUM 残差相对当前 `lockedPose` 计算，绝对漂移租绳相对锁定进入时的 `anchorOrigin` 计算，解锁时记录 `lockedPose` 相对当前候选输出的接缝残差，后续在独立接缝阶段仅通过该残差衰减回归候选输出；本地低分重获取链路的几何仲裁不得直接复用 Python 评分层的深度有效性判据，Unity 当前真实实现应按 `color_reprojection>=0` 判颜色有效，并按 `depth_valid_in_mask>=0.10` 判深度有效，再在本地对 `ScoreReprojection/ScoreDepth` 做加权对数几何平均；深度有效信号 `\chi_t^{(d)}` 的布尔公式必须保持合法，表达“存在渲染深度状态且内点比例或残差信号至少一路有效”这一语义时，优先使用布尔乘积求补或等价的容斥表达，不得把布尔“或”直接写成普通线性相加；状态机条款必须贴合 `AnchorStateMachine` 当前真实结构，不得回写旧的“跳变/掩膜缺失/连续低重投影触发失败”主状态机；为避免命令应答载荷 `a_s` 与状态机可靠观测标志混淆，专利稿中命令应答保持 `a_t/a_{acc}/a_{rej}` 系，状态机接受可靠观测标志统一使用 `g_t`；运动模型条款应优先写成“控制点 + 预测算子”接口结构，再把恒速积分作为具体实现之一，不要把当前多模型公共接口压缩成单一固定状态方程；说明书后半段不得残留 `latest-only`、`统一输入输出接口串接` 或旧版单一平滑公式等与前文不一致的历史表述；为了增强创造性抗辩，摘要、独立权利要求和发明内容前段应前置写明“外部视觉计算设备仅输出相机坐标系目标位姿与可靠性信息，不直接输出世界锚点位姿；世界锚点仅由头戴端按 `frame_id` 回查采集时刻参考相机世界位姿后刚体组合得到”，避免被写成普通的外部跟踪结果直接配准显示方案；当代理建议补“具体算法”时，应优先补入当前实现中真实存在的缓存维护、内参映射、掩膜筛选、掩膜调制、状态转移、输出策略和静止锚定公式，并同步检查权利要求、发明内容和具体实施方式三处是否一致，不得凭空发明未实现机制。
