...
- **实验三：日常物体上的任务层用户研究**。`2 条件 x 3 物体 x 3 任务 x 2 重复` 被试内设计，只比较 *One-Euro Anchor* 与完整 *EgoAnchor*；当前只冻结设计，实验一/二完成后再启动正式采集。完整方案见 `2026-EgoAnchor/experiment_3_design_zh.md`。
...
IEEE VR 2027 的投稿上限是正文、图和表最多 9 页，参考文献最多另占 2 页。**当前处于撰写阶段，页数不作硬约束**：内容可以适当超出 9 页，先把论述、证据和细节写足，最后统一浓缩到投稿上限。不要为了压页数提前删减实质内容或牺牲论证完整性；也不要因为超页而拒绝补写章节。压缩留到定稿前的专门一轮处理，届时优先压缩公式展开、审计指标叙述和重复的边界说明，而不是删证据或删章节。当前 `egoanchor_cn_v7.tex` 正文占 p1--p8、参考文献起于 p9。实验三是已规划的任务层效用验证，当前只写冻结设计、不占图表版面，待实验一/二完成后再启动正式采集。

实验三的完整冻结设计固定在 `2026-EgoAnchor/experiment_3_design_zh.md`，`plan.md` 只保留摘要、论文 §5.5 只保留可发表叙述；改动前必须先读该文件。要点：条件只有 *One-Euro Anchor* 与完整 *EgoAnchor*，*Arrival-Hold* 只作训练阶段演示、不进入推断统计。交叉核心对象为 `blue_mouse`、`stapler` 与 `gamepad`（覆盖尺寸/纹理/几何三个维度；手柄强制单手握持单侧搬移，使另一手保持握持控制器），`earphone` 作困难样例只由操作员采集。对象是最外层区块，因为 `--object` 只在服务启动时读取、协议无运行时切换动作。三项任务为静止观察、拿起放下、遮挡恢复，相互独立且顺序平衡；主要因变量统一为"系统相关状态转换后首次成功操作的时间"，方法 x 任务交互是预设的确认性检验。T1/T2 **不使用人为探针任务**（读 4 字符码与射线选中小目标均已评估否决：整体游移不影响可读性、单次注视内抖动仅 0.17 个字高，正确率会撞天花板；小目标选择受追踪补偿影响；两者还要额外花版面解释），参与者动作统一为"在判断虚拟内容已稳定附着、可放心用于指示物体上具体位置时按扳机"，即参与者判据 + 客观时间戳。T3 例外保留行为任务并以首次尝试成功率与时间并列，使实验三同时保有参与者判据与纯行为两类客观证据。主观测量是主要构念而非补充，用四个信任/归属单项，措辞避开直接描述操纵的词。**遮挡时长必须落在 coasting 区间（0.6--0.9 s 起，预实验校准后冻结）**：`coastTimeoutSeconds=0.45`、`lostTimeoutSeconds=1.0`，更长的遮挡会使两条件都进入 Lost 并等待同一次服务器 REGISTER（中位数 750.26 ms），使主要因变量主要反映感知后端而非运行时；生命周期状态分布与两条件的候选/VCD/接纳率一致性是必须报告的操纵检验。运行时参数不得为实验三修改。主观测量只用四个信任/归属单项（陈述句 + 7 点同意度，前三项任务后即时填写）加 Raw NASA-TLX 与最终偏好，不得引入 SUS，也不得把单项合并为总分或对两三条目分量表报 Cronbach's alpha；不得引入 CRIQ。样本量 N=24 目标、N=18 下限（平衡组为 6 组，N 必须是 6 的倍数），由预实验校准难度与方差，不得用实验一的片段级中位数推导 n 或断言感知阈值。超时按删失处理：不进入时间模型、单独报超时率、做插补敏感性分析。日常物体上没有同平台参考位姿，实验三不得报告绝对配准误差，只报任务指标、主观评价和无需真值的自参考稳定性日志，且不得作中介效应主张。Exp3 使用独立的 2 runtime `variant_matrix_id`、独立启动门禁和与 schema-v2 隔离的日志与分析模块，不复用实验一/二的九路矩阵。
...
- `analyze` 只在活动分析目录生成 TeX，不回填主稿；`copy-assets` 按本次 `build_result.json` 和 `batch.toml` 显式发布当前实验面板、指定 replay/relay PNG/PDF 与三张表格 TeX。表格默认发布为 `tables/exp1_static.tex`、`tables/exp1_dynamic.tex` 和 `tables/exp2_design.tex`；图环境 TeX 仍由研究者审阅后手工纳入。主稿编译不属于 `pixi run eval`，稳定交付文件为 `2026-EgoAnchor/pdf/egoanchor_cn_v7.pdf`。
...
- 论文构建使用 `egoanchor_cn_v7` basename；PDF 与辅助产物写入 `2026-EgoAnchor/pdf/`，稳定交付文件为 `pdf/egoanchor_cn_v7.pdf`。工作区的 LaTeX Workshop 输出目录同步设为 `%DIR%/pdf`，不得再固定为无关的 `EgoAnchor` jobname。
