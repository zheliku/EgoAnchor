...
\abstract{%
动态真实物体锚定是实现以真实物体为中心的透视混合现实（Passthrough Mixed Reality，PMR）交互的基础能力。然而，现有方案通常依赖物理标签、专用深度传感器或逐物体离线训练，难以部署到开放消费级场景中的日常物体。本文提出 EgoAnchor，一套面向开放消费级混合现实的动态真实物体锚定系统。系统仅依赖头戴显示器双目图像与目标物体三维模型，无需物理标签、专用深度硬件或逐物体训练，即可实现日常刚性物体的连续6DoF动态锚定。为此，我们构建了一套由视觉感知后端与对象锚定运行时组成的系统架构：前者负责持续生成异步物体位姿流，后者通过时间对齐、质量控制、时序稳定和生命周期管理，将异步位姿流持续转换为世界坐标系下稳定、可恢复的对象锚点。我们在 Meta Quest 3 上完成系统实现，并通过定量实验和用户研究验证了系统性能。结果表明，EgoAnchor 能够有效支持开放消费级混合现实中的动态真实物体锚定，并显著提升锚定稳定性与交互体验。}
...
近年来，计算机视觉领域基础模型（Foundation Models）的快速发展，为开放场景中的真实物体感知提供了新的可能。开放词表目标检测、视频对象分割、立体匹配以及零样本6DoF位姿估计等基础模型不断提升对象感知的泛化能力，使消费级设备逐步具备面向任意日常物体构建动态对象锚定系统的潜力。然而，对象感知（Object Perception）仅关注恢复物体在相机坐标系中的6DoF位姿，而对象锚定（Object Anchoring）则需要进一步在异步视觉推理、设备持续运动、感知噪声以及目标暂时丢失等实际运行条件下，维护世界坐标系中的对象空间参考，为上层应用提供稳定、一致且可恢复的对象锚点。如何将开放对象感知能力组织为可部署的视觉感知流水线，并进一步将异步视觉位姿流持续转换为稳定的对象锚点，仍然缺乏统一的系统支持，这也成为开放消费级混合现实中动态真实物体锚定的关键挑战。

为解决上述问题，本文提出 EgoAnchor，一套面向开放消费级混合现实的动态真实物体锚定系统。系统采用对象感知与对象锚定解耦的分层架构，由视觉感知后端（Visual Perception Backend）和对象锚定运行时（Object Anchoring Runtime）组成。视觉感知后端持续输出异步物体位姿流；对象锚定运行时进一步将异步视觉位姿流转换为世界坐标系下稳定、连续且可恢复的对象锚点，从而为混合现实应用提供统一的动态对象锚定能力。整个系统仅依赖头戴显示器双目图像与目标物体三维模型，无需物理标记、专用深度设备或逐物体离线训练，并能够与近年来的 Image-to-3D 技术结合，为开放场景中的更多日常刚性物体提供部署潜力。

我们在 Meta Quest 3 上实现了 EgoAnchor，并通过定量实验和用户研究对系统进行了评估。定量实验利用官方控制器提供的高精度位姿作为参考，测量系统的锚定精度、稳定性与响应性能；用户研究围绕典型混合现实交互任务展开，评估系统在真实应用中的可用性。实验结果表明，EgoAnchor 能够稳定支持开放消费级混合现实中的动态真实物体锚定，并有效提升真实物体交互的稳定性与整体体验。

本文的主要贡献如下：

\begin{enumerate}

\item 提出 EgoAnchor，一套面向开放消费级混合现实的动态真实物体锚定系统，在仅依赖头戴显示器双目图像与目标物体三维模型的条件下，实现任意日常刚性物体的连续6DoF动态锚定。

\item 设计一套面向开放消费级混合现实的视觉感知后端（Visual Perception Backend），将目标发现、目标分割、双目重建与零样本6DoF位姿估计组织为统一的异步感知流水线，为对象锚定提供统一的异步物体位姿流。

\item 提出对象锚定运行时（Object Anchoring Runtime），通过时间对齐、质量控制、时序稳定和生命周期管理，将异步视觉位姿流持续转换为世界坐标系下稳定、连续且可恢复的对象锚点。

\item 在 Meta Quest 3 上完成系统实现，并通过定量实验和用户研究验证了系统在锚定精度、稳定性、响应性能以及真实交互体验方面的有效性。
...
早期系统主要采用基于物理标记的方法，通过 AprilTag、ArUco 等人工标记获得稳定的位姿估计，ARToolKit、ARCore Augmented Images 等系统均属于这一范式。这类方法实现简单、鲁棒性较高，但需要预先在物体表面贴附标记，不仅改变真实物体外观，也难以适用于开放场景中的任意日常物体。
...
总体而言，现有平台主要提供封装好的对象追踪能力，通常依赖专用硬件、封闭的平台接口或对象级离线准备，难以支持开放场景中的任意日常物体，也较少向开发者开放底层视觉感知结果。因此，如何利用开放视觉感知能力，在消费级设备上构建面向开放场景的动态真实物体锚定系统，仍然具有重要研究价值。
