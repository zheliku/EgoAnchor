...
\title{EgoAnchor：面向透视混合现实的帧对齐真实物体锚定与轻量 One Euro 控制}
...
\abstract{
我们提出 EgoAnchor，一套面向 passthrough 场景的开放、可复现的真实物体锚定系统，仅依赖头戴式显示器的双目相机和物体 3D 模型输入即可实现对目标物体的 6DoF 姿态跟踪，并将其转化为稳定的世界坐标锚定结果。系统由 XR 前端与追踪后端组成，结合双目图像采集、语义定位、深度估计以及对象级 6DoF 姿态跟踪，通过帧对齐锚定机制将异步返回的相机坐标姿态一致映射到对应观测帧的应用世界坐标，从而缓解头动与推理时延耦合导致的空间错位。在 temporal anchor 层，论文计划评估轻量 reliability-gated One Euro 控制：用可靠性和跳变门控过滤不可信观测，再用速度自适应滤波、有界前推、静止锁定和重获取生命周期生成稳定输出。本文将从锚定精度、稳定性、端到端时延与恢复能力等维度对系统进行评估，并对比 raw、low-pass、Kalman、vanilla One Euro 与改进 One Euro，分析异步 6DoF 感知转化为可用 MR 锚定结果的关键条件。}

\keywords{混合现实, 真实物体锚定, 6DoF 姿态跟踪, 异步感知, One Euro 滤波, 低延迟系统}
...
为此，本文提出 EgoAnchor，一套面向 passthrough MR 的真实物体锚定系统。该系统由 XR 前端与追踪后端组成：前端负责采集头戴双目图像、相机信息和观测帧位姿，后端执行语义定位、双目深度估计以及对象级 6DoF 姿态跟踪。EgoAnchor 围绕世界一致、可恢复的真实物体锚定目标组织为一条面向异步感知的系统闭环。该闭环以\textbf{帧对齐锚定}为关键机制；围绕 temporal anchor 层，本文将比较可靠性门控、One Euro 滤波、有界前推和重获取生命周期是否能进一步提升锚定稳定性与恢复行为。
...
\begin{enumerate}
  ...
  \item 基于 ZMQ Protobuf 数据面与 NATS Protobuf 消息/命令面的多 topic 低时延传输；
...
\subsection{轻量 One Euro 锚定更新与恢复}
本节将“从 pose stream 到 anchor behavior”的控制层写成轻量但清晰的决策机制。EgoAnchor 不对每个返回 pose 都直接更新 anchor，而是结合当前观测的可靠性、tracking 状态、frame alignment 结果与 world-space pose jump 决定是否更新。其最小形式包括：
\begin{itemize}
  \item \textbf{输入：} 姿态可靠性、tracking 状态、pose jump、回包年龄；
  \item \textbf{输出：} One Euro Update、Static Lock、Bounded Prediction、Hold、Reacquire 等锚定模式；
  \item \textbf{原则：} 错误更新通常比短时保持更糟；系统应在高不确定性时抑制错误 anchor 更新。
\end{itemize}
当前目标实现采用 One Euro filter 作为滤波核心，并只加入 MR 锚定所需的门控和生命周期，可按以下三个层次展开：
\begin{enumerate}
  \item \textbf{可靠性门控：} 由 pose score、flags、pose 跳变与 track 状态共同构成；
  \item \textbf{滤波与输出：} 可信观测进入 One Euro，静止时锁定输出，运动时做有界前推；
...
本节说明 Unity 与 Python 之间的多 topic ZMQ Protobuf / NATS Protobuf 运行时管线，包括 \texttt{quest\_stereo}、\texttt{quest\_camera\_info}、\texttt{pose}、\texttt{status}、\texttt{heartbeat} 与 command request/reply 的职责，以及 topic-level latest-drain、发送帧 pose 回查和低高水位配置如何共同支撑低时延与世界一致性目标。
...
\begin{itemize}
  ...
  \item \textbf{RQ2：} low-pass、Kalman、vanilla One Euro 和 reliability-gated One Euro 在稳定性、响应延迟和错误更新上有什么差异？
...
\begin{enumerate}[label=\alph*.]
  \item \textbf{条件设置：} Baseline 与 EgoAnchor full。Baseline 至少包括 frame-aligned raw、low-pass、Kalman 和 vanilla One Euro；EgoAnchor full 则包含 reliability-gated One Euro 与恢复策略。
...
\begin{enumerate}
  ...
  \item \textbf{关键消融：} frame alignment、Kalman/One Euro baseline 与 reliability-gated One Euro 更新策略的贡献；
...
\subsection{关键机制消融}
本节集中检验 frame alignment、vanilla One Euro 和 reliability-gated One Euro 的收益，通过快速头动和异常观测条件下的定量与可视化结果说明：为什么错误时刻的坐标映射会造成世界错位，为什么 Kalman 或 vanilla One Euro 不一定能处理坏 pose，以及为什么错误更新往往比短时保持更糟。
...
\subsection{局限性与未来工作}
本节讨论多物体扩展、头显端轻量部署、在线个性化和跨场景长期锚定等方向，并明确区分本文当前目标中的轻量 One Euro controller 与未来更完整的学习式或任务自适应 temporal policy 设计。
...
% 临时输出参考文献，避免在尚未加入 \cite 时触发 BibTeX 的 “I found no \citation commands” 提示。
\nocite{*}
