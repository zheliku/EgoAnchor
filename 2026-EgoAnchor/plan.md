...
诚实边界：不报绝对配准误差、不提供任务表现证据、不作中介效应主张；结论只覆盖当前对象、设备、参数与任务条件。量表原文核对已于 2026-07-26 完成（AQ/TiA/S-TIAS/Gottsacker，见权威文件〇节）；三量表一律以 adapted 署名。剩余前置是确认性认知访谈、预实验参数冻结和 Unity 侧工程实现，不声称执行独立译者盲法回译。正式采集与分析的计划输入路径为 `material/EgoAnchor_Experiment3_RawData_24P_v5_1.xlsx`；该 7 表工作簿由统一 eval 包中的模板构造器生成，`Participants` 与 `Records` 保存人工原始值，`Derived` 和 `Analysis` 只作实时公式派生与现场核对。2026-08-01 来源审计发现该路径下现有文件的 4560 个核心响应与 GPT 合成参考逐格一致，且记录时间晚于审计日期；当前文件只可用于流程演练，必须在正式采集前替换为来源可核验的原始数据。来源门禁采用正向批准：已知合成指纹始终拒绝，其他正式工作簿也要在核验来源后把核心响应指纹登记到受版本控制的批准列表。分析结果簿固定为 6 张中文页，来源门禁未通过时禁止复制论文资源。
