...
诚实边界：不报绝对配准误差、不提供任务表现证据、不作中介效应主张；结论只覆盖当前对象、设备、参数与任务条件。量表原文核对已于 2026-07-26 完成（AQ/TiA/S-TIAS/Gottsacker，见权威文件〇节）；三量表一律以 adapted 署名。剩余前置是确认性认知访谈、预实验参数冻结和 Unity 侧工程实现，不声称执行独立译者盲法回译。正式采集与分析的唯一输入为 `material/EgoAnchor_Experiment3_RawData_24P_v5_1.xlsx`；该 7 表工作簿由统一 eval 包中的模板构造器生成，`Participants` 与 `Records` 保存人工原始值，`Derived` 和 `Analysis` 只作实时公式派生与现场核对。
