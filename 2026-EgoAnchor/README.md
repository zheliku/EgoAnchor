...
最新中文工作稿为 `egoanchor_cn_v2.tex`，使用 IEEE VR / VGTC 模板和 XeLaTeX 编译。
该稿第 3 章当前由 Claude 单独重写，图表任务不得回退或覆盖该章。`egoanchor_cn_ai_v8.tex`、
`egoanchor_cn_v6.tex`、`egoanchor_cn_v7.tex` 与 `egoanchor_cn_v8.tex` 作为旧稿保留。

目录
----

- `egoanchor_cn_v2.tex`：当前中文工作稿。
- `egoanchor_cn_refs.bib`：主稿参考文献。
- `figures/`：系统图、定性 replay、正文组合图和独立审计子图。
- `tables/`：由统一 `copy-assets` 命令按 TOML 显式复制的 LaTeX 表。
- `pdf/egoanchor_cn_v2.pdf`：与工作稿同名的编译结果，供内部审阅和 VSCode/LaTeX Workshop 打开。
- `plan.md`：当前论文路线和实验边界。
...
也可运行 `make`。PDF、XDV、AUX 与日志都使用 `egoanchor_cn_v2` 这一 basename，
