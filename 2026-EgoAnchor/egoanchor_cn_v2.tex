...
\title{EgoAnchor: A Training-Free Real-Object Anchoring System for Passthrough Mixed Reality}
...
\abstract{%
在透视混合现实（Passthrough Mixed Reality, PMR）中，将虚拟内容稳定锚定到物理对象是交互可用性的基础。本文提出 \textbf{EgoAnchor}，一套面向 PMR 的真实物体异步锚定系统：仅依赖头戴显示器的双目相机与物体三维模型，无需额外传感器、无需为物体粘贴物理标签（tag），也无需为每个物体单独训练。该系统首先采集双目图像、使用视觉基础模型进行语义定位、深度估计以及获取真实物体在相机坐标系下的 6DoF 位姿，再通过时间对齐、质量门控、帧率升采样和静止锁定进行生命周期控制，将低频含噪的相机系感知流转化为高频、世界一致、稳定可恢复的锚点输出。我们在 Meta Quest 3 上进行了部署实现，并通过定量评估与三项典型交互任务的用户研究进行检验；结果表明 EgoAnchor 在锚定精度、稳定性、响应时间等性能指标上均表现出色，满足用户日常混合现实应用需求。代码将在论文接收后开源。}

\keywords{混合现实, 真实物体锚定, 6DoF 位姿跟踪, 异步感知, 系统设计, 消费级硬件}
...
透视式混合现实（Passthrough Mixed Reality, PMR）能够将虚拟信息直接叠加到用户所处的真实环境之上，诸如虚拟标签附着、物理对象高亮指导、近物交互辅助等核心交互，都依赖于虚拟图层对真实物理世界的精确贴合与稳定锚定。用户转头、走近、伸手操作时，叠加图层应当始终与真实物体保持一致，避免漂移与抖动。 % 区分 object anchor / spacial anchor

现有 XR 平台已经提供了一系列平台级的空间锚定或对象追踪能力，但这些能力通常依赖平台特定的感知基础设施，并在目标对象类型、部署条件或动态追踪能力方面存在限制。例如，Microsoft Dynamics 365 Guides 提供基于对象锚定（Object Anchoring）的工业级空间对齐能力，可将虚拟内容自动附着到已知物理设备上。然而，该能力依赖预处理的对象模型以及 HoloLens~2 的空间映射与深度感知能力，主要面向静态工业设备，而非消费级透视混合现实场景中的动态日常物体。类似地，仅凭相机与三维模型工作的 Vuforia Model Targets 通过边缘匹配实现免标记追踪，但被明确定位为针对静态物体，物体移动后难以保持稳定跟踪~\cite{vuforiaModelTargets}。

近年来，消费级头显开始尝试支持动态物体追踪。Apple Vision Pro 在 visionOS~2 中将预训练对象追踪扩展到手持和运动物体~\cite{appleVisionOS27ObjectTracking}。然而，该能力仍依赖搭载 LiDAR 的封闭硬件平台，并需要通过 Create~ML 对目标对象进行离线模型生成与预处理。另一方面，Meta Quest 提供了动态对象跟踪接口，但当前主要面向键盘等少量预定义对象类别，且官方文档指出其更新频率并不适用于高速实时运动目标的连续跟随~\cite{metaDynamicObjectTracker,metaSceneOverview}。因此，如何实现依赖性低、可覆盖任意日常刚性物体的动态锚定，仍是一个未被充分解决的挑战。 % 目标与特点待定：因此，如何实现 xxx ，仍是一个未被充分解决的挑战。

实现一个\textbf{仅依赖双目图像输入、无需额外传感器、无需为物体粘贴物理标签（tag）、也无需为特定物体专门训练模型，且能支持任意日常刚性物体动态追踪并部署在消费级硬件上}的系统，具有较高的应用价值。然而，能够同时兼顾这些维度的技术方案在现有研究中仍属空白。

为填补这一空白，我们提出 \textbf{EgoAnchor}，一套仅依赖双目透视图像与物体三维模型的端到端真实物体锚定系统。我们并不提出新的位姿网络，而是将上述预训练视觉基础模型组装为一条\emph{即插即用、无需逐物体训练}的感知前端，从双目图像直接获取物体在相机坐标系下的 6DoF 位姿；系统真正的核心在于其后的锚定运行时，它经由时间对齐、质量门控、时序平滑与生命周期管理四层协同处理，弥合上述目标差异，将低频含噪的相机系感知流转化为世界一致、稳定可恢复的锚点输出。整套感知-锚定回路直接运行于消费级 GPU 之上。系统所需的物体模型亦无需人工建模或专门采集，可由日常手机照片经“图像到三维”（Image-to-3D）生成技术获得~\cite{trellis2024,appleObjectCapture}，因而原则上可覆盖鼠标、耳机、手柄等任意刚性日常物体。

我们通过性能评估和用户研究来验证我们的系统。在性能评估中，为了准确获得物体的真实位姿，我们选取 Meta Quest 3 官方手柄作为测试物体并通过官方 Meta XR SDK 获取其位姿信息，与我们的系统进行对比测试，包含系统的锚定精度、稳定性和响应时间等关键性能指标。在用户研究中，我们选取 3 种日常物体并设计了三个典型交互任务，邀请参与者使用 EgoAnchor 系统进行操作，并收集他们的主观体验和反馈。实验结果表明，EgoAnchor 在锚定精度、稳定性、响应时间等性能方面均表现出色，且能够满足用户日常混合现实应用的需求。

本文贡献如下：
\begin{enumerate}
  \item 提出了 EgoAnchor，一套面向透视混合现实的免训练真实物体锚定系统。系统复用预训练的视觉基础模型作为即插即用的感知前端，在无需额外传感器、无需物理标签、无需逐物体训练的条件下，实现对任意日常刚性物体的 6DoF 姿态跟踪与稳定锚定。
  \item 明确并弥合了 CV 位姿估计与 MR 锚定之间的目标差异——前者优化单帧相机坐标系精度，后者要求世界坐标系下跨时间的一致可用性。我们将这一差异显式拆解为时间、噪声与容错三重挑战，并以一条 pose-to-anchor 管线加以解决。
  \item 设计了一套四层协同锚定运行时，包括时间对齐、质量门控、时序平滑（含静止锁定）与生命周期管理，实现异步相机系感知流向应用层世界锚点的稳定转化。
  \item 在 Meta Quest 3 上实现并验证了该系统，通过定量性能评估与用户研究展示了其在锚定精度、稳定性和响应时间等方面的优越表现与用户可用性，并提供了可复现的实验设置与评估协议，为未来研究提供参考。
\end{enumerate}
...
\subsection{平台级真实物体锚定能力对比}

现有 XR 平台从多个方向支持真实物体锚定，但每条技术路线都在能力维度上有所取舍（见表~\ref{tab:capability-comparison} 详细对比）。基于图像标记或预制参考的方案，如 ARToolKit~\cite{kato1999artoolkit}与 ARCore 的 Augmented Images~\cite{googleArcoreAugmentedImages}，依赖事先注册的平面图案，难以覆盖无标记的三维刚体。
