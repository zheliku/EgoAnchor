...
\abstract{%
在透视混合现实（Passthrough Mixed Reality, PMR）中，将虚拟内容稳定、精确地附着于移动的日常物体上是保障近物精细交互可用性的基础。然而，现有方案通常依赖有损外观的物理标签（Tag）、特定的硬件基础设施、或昂贵的逐物体离线训练。本文提出 \textbf{EgoAnchor}，一套面向 PMR 的第一人称真实物体异步锚定系统。系统仅依赖头戴显示器的双目图像与物体三维模型，通过解耦的“感知-锚定”双端管线实现。其中，视觉追踪后端从双目图像中解算相机系下的 6DoF 位姿；XR 运行前端则通过时间对齐、质量门控、时序平滑与生命周期管理四层协同策略，转化为世界坐标系下空间一致、鲁棒可恢复的世界锚点。我们在 Meta Quest 3 设备上进行了部署，定量评估与包含三项典型交互任务的用户研究表明，EgoAnchor 在锚定精度、稳定性、响应时间等关键指标上均表现出色，充分满足日常混合现实应用需求。代码将在论文接收后开源。}
...
\begin{enumerate}
  \item 提出了 EgoAnchor，一套面向 PMR 的第一人称真实物体异步锚定系统。在仅依赖头戴显示器的双目相机与物体三维模型的条件下，实现对日常刚性物体的 6DoF 姿态跟踪与稳定锚定。
