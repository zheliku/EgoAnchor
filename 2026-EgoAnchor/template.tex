...
\teaser{
  \centering
  \includegraphics[width=\linewidth, alt={A view of clouds with orange sunrays shining through from behind.}]{clouds.jpg}
...
\item The style uses the hyperref package, thus turns references into internal links. We thus recommend using ``\texttt{\textbackslash cref\{reference\}}'' (instead of ``\texttt{Figure\~{}\textbackslash ref\{reference\}}'' or similar) since ``\texttt{\textbackslash cref\{reference\}}'' turns the entire reference into an internal link, not just the number. Examples: \cref{fig:vis_papers} and \cref{tab:vis_papers}.
\item The style automatically looks for image files with the correct extension (eps for regular \LaTeX; pdf, png, and jpg for pdf\LaTeX), in a set of given subfolders (figures/, pictures/, images/). It is thus sufficient to use ``\texttt{\textbackslash includegraphics\{clouds\}}'' (instead of ``\texttt{\textbackslash includegraphics\{figs/clouds.jpg\}}'').
