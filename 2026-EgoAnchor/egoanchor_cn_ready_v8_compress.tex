...
\begin{figure}[t]
  ...
  \caption{实验一连续序列回放（$\Delta t=0$--$5.00\,\mathrm{s}$，间隔 $1.25\,\mathrm{s}$）。上两行为原始透视图像与 Quest 平台跟踪参考，下四行为 Arrival、Capture、One-Euro、EgoAnchor 在相同采集时刻的锚点投影；轮廓和局部轴表示估计的 6-DoF 锚点。四个方法行左侧数值为所示时间窗内相对 Quest 参考的逐帧平移/旋转误差均值。}
