...
- 论文主叙事固定为 `pose estimate != usable MR anchor`；
- RQ1/RQ2 证据完整；
- RQ3 协议冻结但结果未完成；
- Meta、Apple 与专用追踪附件的能力边界通过相关工作、紧凑能力表和讨论定位；跨平台定量比较不属于必做实验；
- 中文主稿正文不超过 8.4 页，为 RQ3 结果保留至少 0.55 页；
- 论文不是投稿终稿，直至 RQ3 得到回答或被正式删除。

平台能力随系统版本、地区与目标对象支持范围变化，不能被当作与 EgoAnchor 对等的算法基线。本文必须引用相应平台或附件的正式文档，明确其对象绑定方式、模型或标记前提、坐标参考和可用性；不得以未核验的产品印象替代能力事实。
...
> 视觉后端给出的异步 6DoF 位姿估计不等同于可由混合现实应用持续消费的对象锚点。EgoAnchor 恢复观测的采集时刻世界语义，评估候选是否适合更新锚点，并控制锚点在静止与运动区制间的显示行为，从而把低频、延迟且质量不均的相机系观测转换为持续存在的世界系对象锚点。

论文不以平台 API 的暂时限制为问题本体，也不把工作表述为将 CV 模型接入 VR。平台原生对象支持范围解释为何应用需要外部感知；开放词表分割与零样本 6DoF 感知说明给定模型的更多刚体可以被定位。二者都不回答视觉观测怎样成为锚点。即使未来平台扩大支持类别，观测时间、候选可靠性和锚点输出行为之间的差异仍然存在。

视觉位姿观测与 MR 对象锚点的语义契约不同：

| 视觉位姿观测 | MR 对象锚点 |
| --- | --- |
| 对应历史图像采集时刻的相机系估计 | 在世界与渲染时刻持续可用的运行时实体 |
| 可偶发错位、遮挡失效或暂时中断 | 错误更新会直接形成可见跳变，失效需要生命周期处理 |
| 低频、异步到达且带质量诊断 | 每个渲染 tick 都需要明确的输出或 hold 行为 |
| 单帧或短时跟踪结果 | 必须处理静止、起停、持续运动与恢复 |

全文据此围绕三个运行时缺口组织：

1. **时间语义缺口**：候选属于图像采集时刻；用到达或渲染时刻的头显位姿复合会在头动时破坏世界关系。
2. **可靠性缺口**：形式合法的候选仍可能因遮挡、深度噪声或几何错位而不应更新锚点。
3. **输出行为缺口**：有效候选仍然低频、延迟且有噪声；单一区制时序策略不能同时定义静止稳定性、起停转换、持续运动和恢复行为。

叙事层级固定为：MR 应用对持续对象锚点的需求；平台与外部感知提供的生态动机；视觉观测与锚点之间的三项语义缺口；frame alignment、VCD 和 StaticLock 的运行时方法；RQ1--RQ3 的证据链。时空对齐是系统架构基础，VCD 与显式静止锚定是需要独立验证的主要技术贡献。

## 4. 贡献定义

### C1：从观测到锚点的运行时契约

EgoAnchor 定义异步视觉观测到对象锚点的运行时边界，通过 `frame_id` 恢复图像时间代理处的相机世界变换，并把观测接纳、时序输出和生命周期置于同一契约中。该贡献不重新定义对象检测或 6DoF 位姿估计器。
...
“平台开放性”“CV 到 VR 的集成”“系统实现与评估”均不单列为贡献。评估只承担证据作用。
...
Meta、Apple 与专用追踪附件不构成 RQ1/RQ2 的方法基线。它们提供的对象关联、环境锚点、控制器追踪或附件追踪能力具有不同的传感器、模型、坐标系、访问权限和产品语义，不能在缺少共同参考的条件下解释为同一估计任务的数值比较。

论文必须包含三项平台定位材料：相关工作中的能力范围说明；一张紧凑能力表；讨论中的适用边界。能力表只列经官方文档或同行评审来源确认的对象绑定语义、目标前提、输出坐标语义和可访问限制，不用厂商宣传语推断精度、稳定性或动态物体支持。

平台上下文实验为可选项，只有同时满足以下条件时才可增加：双方支持同一物理对象及其所需模型或标记；使用同一采集动作和时间窗口；能够获得统一且独立于任一待比较系统的参考；不改变 RQ1/RQ2 的正式采集、分析或页数预算。结果仅作为部署语境的描述性材料，不检验 H1/H2，不支撑 C1--C3，也不得用来替代 RQ3 的多对象证据。任何一项条件不满足时，不采集跨平台数值结果。

讨论必须说明平台方案未作为核心定量基线的原因：对象支持集合及其准备前提不同，内部候选与可靠性状态通常不可见，平台时序模块通常不能关闭，跨硬件记录还会混合传感器、预测和显示管线差异。这些原因限定可比性，不构成对任何平台能力的优劣判断。

## 6. Claim-Evidence 合同

| Claim                                  | 必需证据                                                       | 证据不足时的处理                         |
| -------------------------------------- | -------------------------------------------------------------- | ---------------------------------------- |
| 视觉观测可被正确解释为锚点输入       | frame-aligned/arrival-aligned 的坐标语义诊断与公共运行时契约   | 仅作为架构实现，不宣称精度收益           |
| frame alignment 避免头动造成的坐标错配 | 静止目标、主动头动下的 Frame-aligned/Arrival-aligned 配对诊断  | 不把到达时刻复合称作正确世界对齐         |
...
| 平台能力定位                           | 相关工作、紧凑能力表与讨论中的官方能力来源                    | 仅解释外部感知动机，不形成跨平台优劣主张 |
...
2. Related Work
   ...
   - Platform and dedicated-tracker capability positioning
3. EgoAnchor
...
摘要固定为六个语义单元：MR 应用对持续对象锚点的需求；`pose estimate != usable anchor`；EgoAnchor 的运行时定义；frame alignment、VCD 与 StaticLock；真实 RQ1/RQ2 结果；模型、平台参考和部署边界。平台支持范围只作背景，不占摘要的中心论证。Run 1 使用明确占位符，不写预期趋势。Run 2 只填写数据支持的结论。

引言固定为五段，且不得颠倒顺序：

1. 从标签、引导、控制界面等 MR 应用对稳定、持续、世界一致对象锚点的需求开始，不以平台 API、网络或视觉模型开篇。
2. 说明平台原生追踪提供系统集成而对象范围和运行时行为由平台决定；开放词表分割与零样本 6DoF 感知扩大给定模型刚体的可感知范围。该段只回答外部感知为何必要。
3. 明确视觉位姿与 MR 锚点之间的时间语义、可靠性和输出行为缺口；这段是引言的技术问题中心。
4. 将 EgoAnchor 定义为连接视觉观测和持续世界系锚点的可靠性与区制感知运行时，依次说明 frame alignment、VCD 和 StaticLock，并限定其不重新发明感知后端。
5. 按数据流列出三项贡献和 RQ1--RQ3 的证据安排。平台能力表不出现在贡献列表中。
...
平台能力定位表放在相关工作，压缩为半栏，不计入上述核心方法/结果浮动体。它只列 Meta、Apple 与专用追踪附件的已证实对象绑定语义、目标前提、输出坐标语义和访问限制。
...
- 全文以“视觉位姿估计不等同于可用 MR 锚点”为主定位；不得把平台 API 缺口或“把 CV 接入 VR”写成问题本体、贡献或结论。
- 平台限制只解释外部感知为何必要；视觉感知进步只说明该运行时问题为何现在可处理。二者都不能替代时间语义、可靠性与输出行为的技术动机。
...
- 平台能力表不把产品功能改写为算法性能；没有统一外部参考的跨平台记录不得写成定量优劣比较。
- 以 `zero-shot object perception`、`model-available rigid objects`、`objects beyond platform-supported classes` 或 `extensible perception backend` 描述感知范围；不写 `arbitrary open-world object tracking` 或无条件“开放对象追踪”。
...
2. 冻结“pose estimate != usable MR anchor”的叙事层级、术语、三项贡献、RQ、estimand、配置标签和论文页数。
...
11. 重写 `egoanchor_cn_v6.tex` 的标题、摘要占位版、五段引言、相关工作、方法、实现、RQ1--RQ3 设计、讨论边界和结论占位版；以观测到锚点的语义缺口组织全文，加入经来源核验的平台能力定位表；删除旧结果。
12. 核验并补齐真实参考文献；平台能力表采用官方或同行评审来源；替换 teaser 占位框；结果图使用固定占位接口。
13. 编译主稿并控制在 8.4 页以内。
...
- 论文超过页限或必须靠版式压缩容纳；
- 主叙事退化为平台功能缺口、感知模型集成或未验证的平台优劣；
- RQ3 未回答但投稿稿仍保留 RQ3 或多对象泛化结论。

## 22. 最终边界

两次运行完成后，EgoAnchor 应形成一条可审计的论文证据链：从历史视觉候选、采集时刻世界语义、VCD 证据和公共接纳决定，到四种时序输出、平台参考、事件、分析数据、图表和 LaTeX 数字均逐层可追溯。工程代码不承担旧实验兼容，也不保存与论文主张无关的并行入口。
