...
\input{tables/experiment1_static_occlusion_stability.tex}
\input{tables/experiment1_dynamic_6dof_fidelity.tex}

\textbf{头动下的世界一致性与静止稳定性。} 表~\ref{tab:exp1-static} 同时报告中心化头动泄漏、绝对注册护栏、静止平移帧间增量和遮挡期平移 P95。EgoAnchor 的中心化平移泄漏中位数为 0.82~mm，绝对注册中位数为 6.60~mm，静止帧间增量中位数为 0.06~mm，均为四种方法中的最低值；其遮挡期平移 P95 为 6.00~mm。绝对注册与中心化泄漏分开报告，避免把固定坐标偏置误写成头动抖动。

\textbf{动态 6DoF 保真度。} 表~\ref{tab:exp1-dynamic} 将平移、旋转的有效时延与 lag-aligned RMSE 成对报告，并单列 Start-transition。EgoAnchor 的平移/旋转对齐 RMSE 中位数分别为 9.12~mm 和 4.81~deg，均低于三种基线；同时 Start-transition 为 510.36~ms，说明显式静止锚定以转换响应换取稳定保持。这个量是系统输出时间线的转换代价，不是网络或单次视觉推理时延。
...
\input{figures/figure2_experiment1.tex}
...
\input{figures/figure3_experiment2.tex}
\input{tables/experiment2_design_attribution.tex}
...
\textbf{稳定性与响应性的边界。} StaticLock、历史时序合成与 Smoothed KF Extrapolation 都会共同改变静止波动、轨迹残差和起停响应；本文已将这些量作为联合权衡报告，并把停止后的前向过冲、反向回动与 settling time 保留在审计产物中。主结论只使用当前 v4 批次的五项任务和冻结指标，不把 v3 归档数字或网页回放作为正式证据。
