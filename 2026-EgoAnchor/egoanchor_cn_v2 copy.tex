...
透视式混合现实（Passthrough Mixed Reality, PMR）能够将虚拟信息直接叠加到用户所处的真实环境之上，诸如虚拟标签附着、物理对象高亮指导、近物交互辅助等核心交互，都依赖于虚拟图层对真实物理世界的精确贴合与稳定锚定。用户转头、走近、伸手操作时，叠加图层应当始终与真实物体保持一致，避免漂移与抖动。

实现这种稳定锚定需要系统同时满足多个维度的能力要求，然而现有方案往往在不同维度上有所妥协，未能全面满足日常通用交互的需求：它们或依赖专有深度传感器（如 HoloLens、Vision Pro），或局限于静态物体（如 Vuforia），或仅支持极少数预定义类别（如 Meta）。因此，实现一个**仅依赖双目图像输入、无需额外传感器、无需为物体添加物理标签（tag）或专门训练深度学习模型，且能支持任意日常刚性物体动态追踪并部署在消费级硬件上**的系统，具有较高的应用价值。然而，能够同时兼顾这些维度的技术方案在现有研究中仍属空白。

具体而言，采用主动深度的方案虽然能够提供高质量的三维感知，但高度依赖专有硬件。例如，HoloLens~2 上的 Azure Object Anchors 依赖空间映射的主动深度，但该服务已于 2023 年底停止~\cite{azureObjectAnchors}。Apple Vision Pro 在 visionOS~2 中首次将预训练对象追踪扩展到手持与运动物体~\cite{appleVisionOS27ObjectTracking}，这虽然取得了重要进展，但其依然依赖搭载 LiDAR 的封闭设备、需要数小时的 Create~ML 离线训练，且其高帧率跟踪路径据报告仅约 30\,Hz。另一方面，仅凭相机与三维模型工作的 Vuforia Model Targets 通过边缘匹配实现免标记追踪，但被明确定位为针对静态物体，物体移动后难以保持跟踪~\cite{vuforiaModelTargets}。当现有平台试图支持运动物体时，其覆盖范围往往会收缩到极有限的固定类别：例如 Meta Quest 的动态物体追踪实际仅稳定支持键盘等少数预定义类，且官方说明明确指出其更新率不适合实时跟随运动物体~\cite{metaDynamicObjectTracker,metaSceneOverview}。

对于混合现实应用而言，它们真正消费的并不是计算机视觉（CV）算法输出的、仅针对单帧图像解算的相机坐标系 6DoF 位姿矩阵，而是一个能够在用户头部持续运动、追踪模型异步计算时延、通信抖动以及物理遮挡发生时，依然维持世界一致性（World Consistency）与视觉稳定性（Visual Stability）的真实物体锚点（Real-Object Anchor）。

为填补这一空白，我们提出 \textbf{EgoAnchor}，一套仅依赖双目透视图像与物体三维模型的端到端真实物体锚定系统。系统由 XR 前端与追踪后端组成。后端的可行性建立在近年快速成熟的一批视觉基础模型之上：可提示分割~\cite{kirillov2023sam,sam3_2025,yoloe2025}、零样本立体深度估计~\cite{foundationstereo2025}、以及无需逐物体微调、在测试时即可泛化到新物体的 6DoF 位姿估计~\cite{foundationpose2023,bundlesdf2023}。我们并不提出新的位姿网络，而是将这些预训练模型组合为一条\emph{即插即用、无需逐物体训练}的感知前端，从双目图像直接获取真实物体在相机坐标系下的 6DoF 位姿。系统所需的物体模型亦无需人工建模或专门采集，可由日常手机照片经"图像到三维"（Image-to-3D）生成技术获得~\cite{trellis2024,appleObjectCapture}，因而原则上可覆盖鼠标、耳机、手柄等任意刚性日常物体；整套感知-锚定回路直接运行于消费级 GPU 之上。

然而，仅有高质量的逐帧感知并不足以支撑混合现实锚定，因为二者的优化目标存在本质差异。计算机视觉领域的 6DoF 位姿估计以\emph{单帧、相机坐标系}下的精度为中心，其主流度量（ADD/ADD-S~\cite{hinterstoisser2012add,posecnn2017} 与 BOP 基准~\cite{hodan2020bop}）刻画的都是物体位姿在当前相机系中的逐帧误差；而混合现实锚定追求的是另一个目标——在一个\emph{持续运动的第一人称观察者世界坐标系}中，锚点跨时间保持一致、且每一帧都可用。这一目标差异带来三重新增挑战：其一，感知输出位于相机坐标系，须正确映射到世界坐标系，且因感知低频、异步而存在时间错配；其二，逐帧精确的位姿流在进入持续运动的渲染回路后，仍会因噪声与时延而打滑、抖动；其三，视觉感知不可避免地会遭遇遮挡、出视野与跟踪失败，应用层却要求输出始终可用、可降级、可恢复。换言之，一个在 ADD 指标上近乎完美的位姿流，未经处理直接驱动渲染，在世界空间中依然可能是不可用的。EgoAnchor 的核心贡献正是弥合这一目标差异：将相机坐标系下的低频含噪感知流，转化为世界一致、稳定可用的锚点输出。
...
本文贡献如下：
\begin{enumerate}
  \item 提出了 EgoAnchor，一套面向透视混合现实的免训练真实物体锚定系统。系统将一批视觉基础模型（可提示分割、零样本立体深度、面向新物体的 6DoF 位姿估计）组合为即插即用的感知前端，在无需额外传感器、无需物理标签、无需逐物体训练的条件下，实现对任意日常刚性物体的 6DoF 姿态跟踪与稳定锚定。
