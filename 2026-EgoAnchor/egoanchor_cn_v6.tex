...
% Source CSV SHA-256: 5a919d5118f840334ccdd12b1bd6aa0de102517d4a4bf505af4be57408329c85; Stage 3 TeX SHA-256: a92954ee010ffa47fcda15f8e606bb7f83f20747bbbd5e774fe5e19a1b29cfbd; generator: egoanchor.eval.materialize-v1; file: exp1_numbers.tex
% Source CSV SHA-256: 5a919d5118f840334ccdd12b1bd6aa0de102517d4a4bf505af4be57408329c85; Stage 3 TeX SHA-256: 7a3f5eb06722aec6e8940bb71f5ce33229df78caed887b04bfcc4c5bacdf364c; generator: egoanchor.eval.materialize-v1; file: exp2_numbers.tex
% 由 egoanchor.eval 自动生成，请勿手工修改本区块。
\newcommand{\EAExpOneArrivalHoldContinuousRotationAngularLagPNinetyFiveResidualDeg}{5.159}
...
\newcommand{\EAExpOneArrivalHoldContinuousTranslationTranslationLagPNinetyFiveResidualMm}{18.324}
...
\newcommand{\EAExpOneArrivalHoldOcclusionRecoveryOcclusionOutputCoveragePct}{66.368}
...
\newcommand{\EAExpOneArrivalHoldOcclusionRecoveryReappearanceTranslationPNinetyFiveMm}{2.479}
...
\newcommand{\EAExpOneArrivalHoldStartStopSixDofMotionHoldRatioPct}{85.484}
...
\newcommand{\EAExpOneCaptureHoldContinuousRotationAngularLagPNinetyFiveResidualDeg}{5.127}
...
\newcommand{\EAExpOneCaptureHoldContinuousTranslationTranslationLagPNinetyFiveResidualMm}{18.309}
...
\newcommand{\EAExpOneCaptureHoldOcclusionRecoveryOcclusionOutputCoveragePct}{66.368}
...
\newcommand{\EAExpOneCaptureHoldOcclusionRecoveryReappearanceTranslationPNinetyFiveMm}{2.252}
...
\newcommand{\EAExpOneCaptureHoldStartStopSixDofMotionHoldRatioPct}{85.484}
...
\newcommand{\EAExpOneEgoAnchorOcclusionRecoveryOcclusionOutputCoveragePct}{64.574}
...
\newcommand{\EAExpOneEgoAnchorOcclusionRecoveryReappearanceTranslationPNinetyFiveMm}{1.485}
...
\newcommand{\EAExpOneEgoAnchorStartStopSixDofMotionHoldRatioPct}{30.205}
...
\newcommand{\EAExpOneEgoAnchorStartStopSixDofPostStopPositionJitterRmsMm}{1.033}
...
\newcommand{\EAExpOneOneEuroAnchorContinuousTranslationTranslationLagPNinetyFiveResidualMm}{18.654}
...
\newcommand{\EAExpOneOneEuroAnchorOcclusionRecoveryOcclusionOutputCoveragePct}{66.368}
...
\newcommand{\EAExpOneOneEuroAnchorOcclusionRecoveryReappearanceTranslationPNinetyFiveMm}{1.754}
...
\newcommand{\EAExpOneOneEuroAnchorStartStopSixDofMotionHoldRatioPct}{85.484}
...
\begin{table*}[t]
  \centering
  \caption{实验一按系统属性组织的五场景结果。每个单元格报告 event/segment-level median [IQR]，括号中的 $n$ 为事件或片段数；不同场景不混池。}
  \label{tab:exp1-generated-summary}
  \resizebox{\textwidth}{!}{%
% EGOANCHOR-EXP-ONE-TABLE:BEGIN
% Source CSV SHA-256: f4ac5ec9f114d2d5a02ff6e98452b7ae6123673caa564d872983613af952bad6; Stage 3 TeX SHA-256: 5fe8eadf02c69a65a816e8b20926f75b3b57b8cf60f5c5d0b1347900e11fda63; generator: egoanchor.eval.materialize-v1; file: exp1_tables.tex
% 由 egoanchor.eval 自动生成，请勿手工修改本区块。
...
本批次包含 $\EAExpOneSessionCount$ 个正式 session、$\EAExpOneScenarioCount$ 个场景，每个场景各有一条完成 trial；统计样本是长序列中预先标记的 event/segment。表~\ref{tab:exp1-generated-summary} 因此只作场景内描述，不给出跨场景总分。静止头动时，EgoAnchor 的平移 event-P95 为 $\EAExpOneEgoAnchorStaticHeadMotionTranslationEventPNinetyFiveMm$~mm，低于 Arrival-Hold 的 $\EAExpOneArrivalHoldStaticHeadMotionTranslationEventPNinetyFiveMm$~mm、Capture-Hold 的 $\EAExpOneCaptureHoldStaticHeadMotionTranslationEventPNinetyFiveMm$~mm 和 One-Euro Anchor 的 $\EAExpOneOneEuroAnchorStaticHeadMotionTranslationEventPNinetyFiveMm$~mm。其位置 HP--RMS 为 $\EAExpOneEgoAnchorStaticHeadMotionPositionHpRmsMm$~mm，而 One-Euro Anchor 为 $\EAExpOneOneEuroAnchorStaticHeadMotionPositionHpRmsMm$~mm。图~\ref{fig:exp1-generated-events}A 显示，Arrival-Hold 的误差尖峰与头动同时出现；采集时刻对齐先消除了这类坐标复合错配，完整系统再限制剩余波动。

这一优势不能直接外推为更快的动态响应。起停 6DoF 中，EgoAnchor 的可见响应时间为 $\EAExpOneEgoAnchorStartStopSixDofVisibleResponseMs$~ms，Arrival-Hold 和 One-Euro Anchor 分别为 $\EAExpOneArrivalHoldStartStopSixDofVisibleResponseMs$~ms 与 $\EAExpOneOneEuroAnchorStartStopSixDofVisibleResponseMs$~ms。停止后经过 1~s guard 的固定 3~s 窗内，EgoAnchor 位置抖动 RMS 为 $\EAExpOneEgoAnchorStartStopSixDofPostStopPositionJitterRmsMm$~mm，也未优于 One-Euro Anchor 的 $\EAExpOneOneEuroAnchorStartStopSixDofPostStopPositionJitterRmsMm$~mm。持续平移中，EgoAnchor 在 $\EAExpOneEgoAnchorContinuousTranslationEffectiveTranslationLagMs$~ms 有效时延下把 lag 补偿 P95 降至 $\EAExpOneEgoAnchorContinuousTranslationTranslationLagPNinetyFiveResidualMm$~mm；Arrival-Hold 为 $\EAExpOneArrivalHoldContinuousTranslationEffectiveTranslationLagMs$~ms 和 $\EAExpOneArrivalHoldContinuousTranslationTranslationLagPNinetyFiveResidualMm$~mm。持续旋转则没有相同收益：EgoAnchor 的角 lag 补偿 P95 为 $\EAExpOneEgoAnchorContinuousRotationAngularLagPNinetyFiveResidualDeg$~deg，高于 Capture-Hold 的 $\EAExpOneCaptureHoldContinuousRotationAngularLagPNinetyFiveResidualDeg$~deg。

遮挡结果体现的是失效约束而非始终输出。EgoAnchor 的遮挡窗平移 P95 为 $\EAExpOneEgoAnchorOcclusionRecoveryOcclusionTranslationPNinetyFiveMm$~mm，低于 One-Euro Anchor 的 $\EAExpOneOneEuroAnchorOcclusionRecoveryOcclusionTranslationPNinetyFiveMm$~mm 和 Arrival-Hold 的 $\EAExpOneArrivalHoldOcclusionRecoveryOcclusionTranslationPNinetyFiveMm$~mm；对应遮挡期 output coverage 为 $\EAExpOneEgoAnchorOcclusionRecoveryOcclusionOutputCoveragePct\%$，三类基线均为 $\EAExpOneArrivalHoldOcclusionRecoveryOcclusionOutputCoveragePct\%$。目标重新可见后固定 1~s 窗内，EgoAnchor 的平移 P95 为 $\EAExpOneEgoAnchorOcclusionRecoveryReappearanceTranslationPNinetyFiveMm$~mm。该结果说明生命周期策略用短暂的输出不可用换取了对有害更新的限制，而不是在遮挡期间维持了更高覆盖率。

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/generated/exp1_behavior_overview.pdf}
  \caption{实验一代表性轨迹与 event-level 汇总。A：静止目标下的头动速度和显示平移误差；B：起停 6DoF 的平台参考、四系统显示位移、运动窗及固定停止后评估窗；C：持续平移中每个 event 的有效时延与 lag 补偿 P95，粗标记和误差棒为 median [IQR]；D：遮挡前后显示误差，底部短线标出 EgoAnchor output 不可用时刻。轨迹只用于解释系统行为，统计单位仍是 event/segment。}
...
\begin{table*}[t]
  \centering
  \caption{实验二单组件机制归因的 event-level 配对结果。表中同时报告完整系统、对应消融和消融减完整系统的 median [IQR]；方向计数为正/零/负差值数量，护栏列保留同一配对的代价或可用性检查。}
  \label{tab:exp2-generated-summary}
  \resizebox{\textwidth}{!}{%
% EGOANCHOR-EXP-TWO-TABLE:BEGIN
% Source CSV SHA-256: f4ac5ec9f114d2d5a02ff6e98452b7ae6123673caa564d872983613af952bad6; Stage 3 TeX SHA-256: 1530608aee023882364dc3c7ffe8c2fccd577e17065aee4ffad1e643c0a9bb63; generator: egoanchor.eval.materialize-v1; file: exp2_tables.tex
% 由 egoanchor.eval 自动生成，请勿手工修改本区块。
...
表~\ref{tab:exp2-generated-summary} 汇总三个适用场景中的同 event 配对。关闭采集时刻对齐后，静止头动平移 event-P95 的中位差值为 $\EAExpTwoCaptureTimeAlignmentTranslationEventPNinetyFiveMmDeltaMedian$~mm，方向计数为正 $\EAExpTwoCaptureTimeAlignmentTranslationEventPNinetyFiveMmPositiveCount$、零 $\EAExpTwoCaptureTimeAlignmentTranslationEventPNinetyFiveMmZeroCount$、负 $\EAExpTwoCaptureTimeAlignmentTranslationEventPNinetyFiveMmNegativeCount$；关闭 StaticLock 后，位置 HP--RMS 差值为 $\EAExpTwoStaticLockPositionHpRmsMmDeltaMedian$~mm，四个配对事件均为正。这分别解释了实验一中的头动世界一致性和静止稳定性。

关闭 VCD 后，遮挡窗平移 P95 差值为 $\EAExpTwoVcdAdmissionOcclusionTranslationPNinetyFiveMmDeltaMedian$~mm，方向计数为正 $\EAExpTwoVcdAdmissionOcclusionTranslationPNinetyFiveMmPositiveCount$、零 $\EAExpTwoVcdAdmissionOcclusionTranslationPNinetyFiveMmZeroCount$、负 $\EAExpTwoVcdAdmissionOcclusionTranslationPNinetyFiveMmNegativeCount$。实际接纳 eligible 子集覆盖 $\EAExpTwoActualAdmittedCoveragePct\%$（$\EAExpTwoActualAdmittedCandidateCount$/$\EAExpTwoEligibleCandidateCount$），P95 risk 为 $\EAExpTwoActualAdmittedTailRiskMm$~mm；该点不反推未记录的运行时阈值。VCD 与随机顺序的 mean-risk AURC 分别为 $\EAExpTwoVcdMeanRiskAurcMm$ 和 $\EAExpTwoRandomMeanRiskAurcMm$~mm，只表征本对象和本次遮挡协议。

时序合成以运动窗近零保持比例为主指标，因为它直接对应低频异步观测之间是否退化为零阶保持。关闭时序合成后，该比例的配对中位差值为 $\EAExpTwoTemporalSynthesisMotionHoldRatioPercentagePointDeltaMedian$~pp（正/零/负方向为 $\EAExpTwoTemporalSynthesisMotionHoldRatioPctPositiveCount$/$\EAExpTwoTemporalSynthesisMotionHoldRatioPctZeroCount$/$\EAExpTwoTemporalSynthesisMotionHoldRatioPctNegativeCount$）；正差值表示消融产生更多近零保持。运动窗平移 P95 的护栏差值为 $\EAExpTwoTemporalSynthesisMotionTranslationPNinetyFiveMmDeltaMedian$~mm，停止后 jitter RMS 与可见响应的配对差值分别为 $\EAExpTwoTemporalSynthesisPostStopPositionJitterRmsMmDeltaMedian$~mm 和 $\EAExpTwoTemporalSynthesisVisibleResponseMsDeltaMedian$~ms。跳变 P95/P99 仍保留在分析 CSV 与审阅工作簿中，但不作为该组件的唯一证据；持续运动的误差--时延权衡仍由实验一报告。

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/generated/exp2_mechanism_attribution.pdf}
  \caption{实验二机制归因图。四个小面板分别显示冻结主指标中完整 EgoAnchor 与对应单组件消融的同 event 配对点，灰线连接相同事件，面板内保留独立单位和差值中位数；右下角 inset 为 VCD 的 P95 tail-risk 曲线，并以星标显示实际接纳 eligible 子集工作点。所有比较均为 event-level，工作点仅作候选级描述。}
  \label{fig:exp2-generated-summary}
\end{figure*}

\subsection{评价指标与汇总}

所有序列均以 display pose 相对于同一 Quest 平台参考的平移和旋转误差为基础。静止头动报告绝对误差、漂移与高通 RMS；起停序列报告运动窗误差、峰值误差、跳变、可见响应，以及停止后 1~s guard 加固定 3~s 窗内的位置抖动；遮挡序列报告遮挡窗误差与 output coverage，并在目标重新可见后的固定 1~s 窗内计算误差。

持续运动的\emph{有效时延}在每个 event 内通过冻结 lag 网格搜索得到：平移通道最小化 $\mathrm{display}(t)$ 与 $\mathrm{reference}(t-\mathrm{lag})$ 的 RMSE，旋转通道最小化对应角残差，并要求满足连续区间和最小重叠契约。选定时延后，同一重叠窗的残差 P95 用于表征去除整体时间偏移后的轨迹保真度。该量是相对于平台参考轨迹的描述性拟合时延，不等同于视觉候选的网络到达时延或运行时主动设置的插值延迟。VCD risk--coverage 是候选级评分诊断，独立于 event-level 系统比较，不用于候选级显著性推断。
