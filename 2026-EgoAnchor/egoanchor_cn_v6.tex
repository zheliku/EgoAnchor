...
\IfFileExists{generated/exp1_numbers.tex}{%
  本批次包含 $\EAExpOneSessionCount$ 个正式 session，覆盖五类预定义物理任务；四种端到端配置各形成 $\EAExpOneEgoAnchorNTrials$ 个配对 trial，显示覆盖率和输出覆盖率均为 $\EAExpOneEgoAnchorDisplayCoveragePct\%$。EgoAnchor 的平移误差中位数为 $\EAExpOneEgoAnchorTranslationMedianMm$~mm、P95 为 $\EAExpOneEgoAnchorTranslationPNinetyFiveMm$~mm，旋转误差中位数为 $\EAExpOneEgoAnchorRotationMedianDeg^\circ$、P95 为 $\EAExpOneEgoAnchorRotationPNinetyFiveDeg^\circ$。四种配置没有在所有指标上形成单一优胜者：Capture-Hold 的平移中位数最低，而 EgoAnchor 的旋转 P95 最低；这说明端到端运行时需要在时间语义、平滑和稳定性之间作联合权衡，而不能只依据单个静态误差指标排序。
