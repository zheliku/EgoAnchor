...
\abstract{%
以真实物体为中心的混合现实应用需要能够随物体运动持续更新的对象级空间参考。然而，异步视觉后端输出的六自由度位姿对应历史图像采集时刻，候选质量随遮挡与几何条件波动，且低频、含噪的位姿流不能直接满足逐帧渲染对稳定性、连续性与生命周期管理的要求。本文提出 EgoAnchor，一套面向消费级混合现实的端到端动态真实物体锚定系统。系统仅依赖头戴显示器双目图像和目标物体的可用三维模型，无需物理标记、物体侧追踪硬件或逐物体训练。EgoAnchor 通过采集时刻世界对齐恢复异步观测的时间语义，以可视度门控的颜色--深度一致性评分限制有害更新，并通过 Kalman 状态估计、Linear/SLERP 时序合成、显式静止锚定与生命周期管理生成应用可持续消费的世界系对象锚点。我们在 Meta Quest 3 上实现系统，并在同一视觉候选流、渲染时间线和平台参考下进行配对系统表征。静止头动时，移除片段固定注册偏置后的 EgoAnchor 平移 P95 为 1.069~mm，而 Arrival-Hold 为 22.026~mm；其静止帧间增量 P95 为 0.072~mm，而 One-Euro Anchor 为 0.431~mm。持续平移时，其有效时延为 320~ms，lag-aligned residual 为 5.409~mm，低于 Arrival-Hold 的 11.670~mm；遮挡窗内，其 episode-level 平移 P95 为 4.645~mm。上述收益伴随 452.7~ms 的起停转换响应，呈现了稳定性与响应性的明确权衡。上述数值描述同一设备与时间线下相对于平台参考的系统行为，并非外部物理真值。
}

\keywords{透视混合现实, 动态对象锚定, 6DoF 位姿估计, 动态物体追踪, 6D 物体定位}

% GPT v4 reproduced from immutable Stage 1 XLSX; input SHA-256: task_1_complete.xlsx=14ac485e397100972f82182039f42f1b9b408db1602cbb3a01b1b4dde4ce8066, task_2_complete.xlsx=3ea23d35c3b5ae4bfee6b1e3abfac3d3b71c2769e3ebac1dab144a35c62db351, task_3_complete.xlsx=f4e997cf8567badfb0c83ab4c2553a19ad80cb7b2de0b5a171ee8525b2a4eaff, task_4_complete.xlsx=08e9ef5767cbc18466afc93811d7903c72b81dceb80da0a0e994166cc2ad8e45, task_5_complete.xlsx=ad06bbff9cc17e00dc9816cb221e198ba519ee39b900689476204b4b9d562c02
\begin{document}
...
运动估计采用常速度卡尔曼滤波~\cite{kalman1960new}。状态向量 $\mathbf{x} = [\mathbf{p}, \mathbf{v}]^T$ 联合编码位置 $\mathbf{p} \in \mathbb{R}^3$ 与速度 $\mathbf{v} \in \mathbb{R}^3$，通过预测-更新循环实现位置与速度的联合估计。位置通道与旋转通道分别维护独立的状态估计器，旋转通道在对数映射空间执行相同的常速度滤波；位置和旋转观测噪声协方差均为冻结配置，VCD 分数只用于候选接纳，不在线调节滤波器噪声。
...
\textbf{头动下的世界一致性与静止稳定性。} 移除每个动作片段的固定注册偏置后，EgoAnchor 的中心化平移 P95 为 1.069~mm，而 Arrival-Hold、Capture-Hold 与 One-Euro Anchor 分别为 22.026、7.891 与 6.726~mm。EgoAnchor 的绝对注册 P95 为 4.169~mm；其静止帧间位置增量 P95 为 0.072~mm，One-Euro Anchor 为 0.431~mm。

\textbf{持续运动中的时延--轨迹质量权衡。} 持续平移中，EgoAnchor 的有效时延 / lag-aligned RMSE 为 320~ms / 5.409~mm；Arrival-Hold 为 182.5~ms / 11.67~mm，One-Euro Anchor 为 390~ms / 6.111~mm。结果支持稳定优先的连续轨迹合成，而不是最低时延主张。

\textbf{持续旋转。} EgoAnchor 的有效时延 / 对齐角 RMSE 为 342.5~ms / 3.81$^\circ$，One-Euro Anchor 为 405~ms / 3.386$^\circ$。旋转结果与平移结果分开报告，避免用位置通道的收益替代姿态通道证据；Hermite 与 Linear/SLERP 的直接比较在实验二的候选筛选中报告。

\textbf{遮挡期间的失效控制。} 遮挡过程中，EgoAnchor 的 episode-level 平移 P95 中位数为 4.645~mm，One-Euro Anchor 为 4.695~mm。完整分布、40~mm 阈值超限率和最大值共同保留在图和审阅表中。

\textbf{起停转换代价。} Start-transition response 使用片段前 250~ms 基线、5~mm 位移阈值和 100~ms 持续条件。EgoAnchor 的片段中位数为 452.7~ms；该量包含 StaticLock 解锁证据、候选更新和延迟合成时间线，不是网络或视觉推理的原始时延。
...
\textbf{采集时刻对齐。} 对同一批原始候选直接应用两种世界复合后，片段级 candidate P95 由 arrival-time 的 22.933~mm 降至 capture-time 的 9.457~mm；11/11 个片段改善，配对中位降幅为 12.228~mm。

\textbf{StaticLock。} 关闭 StaticLock 后，中心化静止 P95 从 1.069 增至 7.782~mm，配对差值为 +6.428~mm。该结果表明 StaticLock 限制慢速静止漂移，而逐帧增量仍作为表格护栏报告。

\textbf{VCD 接纳。} 启用 VCD 时，0/12 次遮挡过程超过 40~mm；关闭后为 6/12。该组件的主证据是尾部失效率，不是单独的中位数。

\textbf{时序合成。} Kalman Predict-to-Now 的 fitted lag / lag-aligned RMSE 为 170~ms / 32.569~mm，正式 Kalman Linear/SLERP 为 320~ms / 5.409~mm。Predict-to-Now 的配对时延变化为 -147.5~ms，但对齐残差增加 27.674~mm；这表明自适应历史目标时刻主要用于换取轨迹保真度，而不是降低显示延迟。Hermite 作为相同模型、接纳、目标时间与生命周期下的插值器对照单独报告。

\textbf{插值器选择。} 在模型、VCD、StaticLock、自适应目标时间和生命周期均相同的条件下，Linear/SLERP 的平移 aligned RMSE 为 5.409~mm，Hermite 为 5.735~mm，Hermite--Linear 配对中位差为 +0.359~mm，29/30 个片段中 Linear 更低；两者的旋转 aligned RMSE 分别为 3.81$^\circ$ 与 3.859$^\circ$，差值为 +0.054$^\circ$，10/14 个片段中 Linear 更低。Hermite 相对 Linear 的平移时延、静止帧间增量和起停响应配对中位变化分别为 0~ms、0~mm 和 0~ms。二者在这些护栏上没有可见差异，而 Linear/SLERP 的残差略低，因此本文将 Linear/SLERP 冻结为 EgoAnchor 的正式输出策略。
...
\textbf{稳定性与响应性的边界。} StaticLock 和历史时序合成在本批次描述性指标中降低了静止抖动与平移轨迹残差，但完整系统的 Start-transition response 为 452.7~ms，持续旋转 aligned RMSE 为 3.810$^\circ$，仍高于 One-Euro Anchor 的 3.386$^\circ$。这些结果限制了快速抓取和直接操控任务中的主张，也说明旋转状态估计仍需独立优化。
