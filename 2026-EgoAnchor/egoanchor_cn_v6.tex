...
\begin{table*}[t]
\centering
\caption{实验二的组件归因与时序策略比较设计。组件消融的效应为关闭配置减完整系统；Smoothed KF Extrapolation--Hermite Interpolation 行只比较关闭 StaticLock 后的两种输出策略。数值待 v4 自动回填。}
...
输出策略 & fitted lag / aligned RMSE & \multicolumn{3}{c}{Smoothed KF Extrapolation vs. Hermite Interpolation；待 v4 自动回填} \\
输出策略护栏 & lag / residual / 起停护栏 & \multicolumn{3}{c}{Smoothed KF Extrapolation vs. Hermite Interpolation；待 v4 自动回填} \\
\bottomrule
\end{tabular}%
\end{table*}
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[t]{0.40\textwidth}
    \centering
    \fbox{\parbox[c][25mm][c]{0.95\linewidth}{\centering Smoothed KF Extrapolation / Hermite Interpolation\\v4 待回填}}
    \caption{时序策略}
    \label{fig:exp2-temporal}
  \end{subfigure}
  \caption{实验二的组件归因与时序策略比较。图~(a)、(b) 分别比较采集时刻复合和 StaticLock；图~(c) 展示 VCD 分数诱导的 event 级风险--覆盖率，同分候选整组进入曲线；图~(d) 配对比较 Smoothed KF Extrapolation 与 Hermite Interpolation。两路共享关闭 StaticLock 的配置。图和数值待 v4 自动回填。}
...
\textbf{Smoothed KF Extrapolation / Hermite Interpolation。} 两路策略在模型、接纳序列和关闭 StaticLock 状态一致时，比较有限时域外推与历史 Hermite 插值的显示代价。除 lag--residual 外，分析还报告候选生效边界步长、静止帧间增量、停止前向过冲、反向回动、settling time、旋转误差及遮挡超限。候选生效边界步长定义为 \texttt{source\_frame\_id} 改变前后相邻两个渲染输出的位姿差，它包含该帧间的真实运动，只作为同时间线的配对显示护栏，不解释成 Kalman innovation。正式结论待 v4 自动回填。
