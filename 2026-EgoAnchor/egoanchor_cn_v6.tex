% Generated from 2026-EgoAnchor-Typst/egoanchor_cn_v6.typ.
...
\abstract{%
动态真实物体锚定是透视混合现实（Passthrough Mixed Reality, PMR）中以真实物体为中心进行空间叠加的关键能力：虚拟内容需实时、连续地附着于真实物体，并随其运动保持世界一致的六自由度对应关系。现有方案通常依赖物理标记、专用传感器、平台级受限接口，或针对单个物体的离线准备流程，难以扩展到开放消费级场景中的日常刚性物体。本文提出 EgoAnchor，一套仅依赖头戴显示器双目图像与目标物体可用三维模型的开放动态物体锚定系统，无需物理标记、专用深度硬件或逐物体离线训练。系统采用感知与锚定解耦的分层架构：视觉感知后端持续生成异步物体位姿流，对象锚定运行时通过时空对齐、运动估计与静止锚定，将低频、含噪且带传输时延的观测转换为世界坐标系下高帧率、连续且可恢复的对象锚点，并依据运动状态在静止与运动区制间切换行为。我们在 Meta Quest 3 上实现 EgoAnchor，并建立覆盖静态锚定质量、动态锚定能力与应用泛化能力的评估框架，评估运动期锚点的连续性与瞬时精度，并对照连续锚定与朴素零阶保持的权衡。
}
...
现有系统已经从不同路径尝试支持对象级追踪与锚定。典型方案包括基于物理标签的跟踪、依赖深度或专用传感器的感知方法、平台提供的对象追踪接口，以及基于 CAD 或三维模型的商业对象识别框架。这些方法在受控环境中能够取得较好的效果，但仍难以同时满足开放消费级 PMR 场景的三个要求：第一，系统应具备开放性，能够在无需逐物体训练的前提下扩展到多类日常刚性物体；第二，系统应具备可部署性，能够在不依赖物理标签、专用深度硬件或受限平台接口的条件下运行于消费级头戴设备；第三，系统应将开放场景中的异步物体感知组织为世界坐标系下连续、一致且可恢复的 6DoF 对象锚点，而不仅仅是输出瞬时物体位姿。

实现这一目标需要同时解决两个问题。首先，系统必须将开放场景中的对象感知组织为可持续输出的异步物体位姿流，而不是一次性的检测或单帧位姿估计。其次，系统必须在设备持续运动、感知噪声、帧率不一致和短时目标丢失等实际运行条件下，将这些异步位姿估计转换为世界坐标系下连续且可恢复的对象锚点。前者决定系统能否面向开放日常物体进行感知，后者决定感知结果能否被 PMR 应用可靠地用于空间附着。
...
我们在 Meta Quest 3 上实现了 EgoAnchor，并建立定量评估与应用评估两条实验轨道。定量评估以标定后的官方控制器平台位姿为参考，评估静态锚定质量、运动期锚定的连续性与瞬时精度，以及连续锚定相对朴素零阶保持的权衡；应用评估围绕典型混合现实任务考察系统在日常物体上的可用性。

本文的主要贡献如下：

\begin{enumerate}
  \item \textbf{EgoAnchor：} 一套面向开放消费级混合现实的真实物体锚定系统，仅依赖头戴显示器双目图像与目标物体的可用三维模型，即可为多类日常刚性物体建立世界一致、可恢复的 6DoF 对象锚点，无需物理标记、专用深度硬件或逐物体离线训练。
  \item \textbf{区制感知的分层锚定方案：} 一套由视觉感知后端与对象锚定运行时组成的分层技术方案。前者持续生成统一的异步物体位姿流；后者通过时空对齐、运动估计与静止锚定，依据运动状态切换锚定行为------在静止与低速区制冻结输出以抑制抖动，在运动期恢复平滑追踪并在目标丢失后重获取。
  \item \textbf{系统实现与评估：} 我们在 Meta Quest 3 上完成 EgoAnchor 系统实现，建立覆盖静态锚定质量、运动期锚定的连续性与瞬时精度及典型 PMR 应用任务的评估方案，并量化连续锚定相对朴素零阶保持在连续性与瞬时精度上的权衡。
\end{enumerate}
...
解锁瞬间，系统以指数衰减方式融合锁定位姿与平滑输出之间的残差，以维持轨迹连续。静止锚定在静止期冻结输出以抑制抖动，运动开始时解锁并恢复平滑追踪：其静止稳定性由 RQ1 评估，运动期轨迹连续性与瞬时精度由 RQ2 评估。
...
\textbf{RQ1：静态锚定质量（Static Anchoring Quality）} --- EgoAnchor 能否在静止场景下提供稳定、精确且鲁棒的对象锚点？显式静止状态建模相对于运动无关平滑策略有何优势？

\textbf{RQ2：动态锚定能力（Dynamic Anchoring Capability）} --- 在所定义的中低速平移与旋转范围内，EgoAnchor 能否将低频异步观测转换为连续的实时 6DoF 锚点？相较 \emph{ZOH}，连续性与渲染时刻精度呈现何种权衡？

\textbf{RQ3：应用泛化能力（Application Generalization）} --- EgoAnchor 在多类日常刚性物体与典型混合现实应用场景中的泛化能力如何？
...
\textbf{精度} --- 锚点输出相对平台参考位姿的平移误差（mm）与旋转误差（°）。中位数描述典型误差，P95 描述尾部误差。

\textbf{静止稳定性} --- 在平台参考判定为静止的连续有效片段内（参考线速度不超过 0.03 m/s），以 1 Hz 二阶 Butterworth 零相位高通分离位置与姿态轨迹中的快速波动，并报告高通残差均方根（high-pass RMS, HP-RMS）。姿态残差在 SO(3) 切空间中计算。

\textbf{运动连续性} --- 以显示更新率（Hz）、未更新帧对占比与锚点输出覆盖率描述运动期输出。未更新帧对是指相邻渲染帧的显示位姿未发生可测变化；锚点输出覆盖率是具有有效锚点输出的纳入渲染帧比例。

\textbf{最新观测时延} --- 定义为当前渲染时刻与最近一次有效视觉观测采集时刻之差，报告中位数与 P95。

\textbf{遮挡鲁棒性} --- 在遮挡恢复任务的完整录制片段上报告位姿误差 P95 与各生命周期状态的时间占比。日志未单独记录目标重新出现时刻，因此不计算恢复时间。

\subsubsection{静态实验场景}


RQ1 评估系统在静止场景下的锚定能力。实验包括两个测试场景：（1）\textbf{长时静止观察}------控制器静置桌面，用户正常头部运动；（2）\textbf{遮挡恢复}------用户手部遮挡目标后移开。长时静止观察报告绝对精度与静止抖动；遮挡恢复报告误差尾部与生命周期状态占比。

完整配置 \emph{Full} 在三个研究问题中保持不变。RQ1 在两个静止场景内同帧记录 \emph{Full} 与 \emph{No-StaticLock}；后者只关闭静止锚定，是用于归因单个机制的消融变体。RQ2 的 \emph{ZOH} 则是系统级参照，用于比较完整连续锚定与离散观测保持，不与 \emph{No-StaticLock} 强行统一。

\subsubsection{动态实验设计}\label{sec:dynamic-design}


RQ2 包含受控往复平移与交替轴向旋转，不再按名义快慢拆分平移试次。按键只标记试次边界，实际运动片段由平台参考速度自动提取：速度轨迹经七帧居中中值滤波后，以名义目标速度的 20\% 为活动阈值，并设置平移 0.02 m/s、旋转 10°/s 的下限；算法合并不超过 200 ms 的短暂停顿，并剔除短于 300 ms 的活动片段。两个平移试次的名义速度为 0.1 与 0.8 m/s，旋转试次为 90°/s。最终分析保留线速度不超过 0.8 m/s、角速度不超过 180°/s 的新鲜有效帧。该范围根据本次参考速度分布事后确定，只用于描述本次数据，不代表系统运行边界。

每个渲染时刻同步记录 \emph{Full} 与 \emph{ZOH}。\emph{ZOH}（zero-order hold，零阶保持）保留最近一次被接受的帧对齐观测，直至下一次观测到达；两种配置采用相同的质量门控、生命周期阈值，以及丢失后保留末次显示位姿的规则，区别在于 \emph{ZOH} 不执行运动估计、时序插值或静止锚定。相邻显示帧的位置和旋转变化同时不超过 1 μm 与 $10^{-4}$° 时，该帧对记为未更新，否则记为更新。未更新帧对占比为其在合格帧对中的比例；显示更新率以更新帧对数除以累计时长。瞬时精度取显示位姿相对同刻平台参考位姿的平移或旋转误差。时间线先选择 trial 中位速度最接近该运动类型总体中位速度的试次，再从其最长连续有效段中央截取 120 个渲染帧；选窗不读取系统输出或误差，横轴使用真实渲染时间戳并辅助标出绝对 Unity 帧范围。
...
针对 \textbf{RQ3}，我们在多类日常刚性物体上开展应用评估。物体集覆盖尺寸、表面纹理与几何对称性三个维度：（1）\textbf{游戏手柄}（中尺寸、强纹理、低对称）------复杂几何与丰富表面细节；（2）\textbf{咖啡杯}（中尺寸、弱纹理、高对称）------挑战弱纹理与圆柱对称；（3）\textbf{笔记本电脑}（大尺寸、中纹理、低对称）------验证大尺寸物体追踪稳定性；（4）\textbf{无线鼠标}（小尺寸、中纹理、低对称）------测试小尺寸物体精度。每个物体配备三维模型（通过 photogrammetry 或现有资源获取）。
...
\textbf{Task 2：多物体高亮引导} --- 桌面上放置 4 个物体（手柄、咖啡杯、鼠标、笔记本），系统依次高亮目标物体，用户按指示顺序拿起并放回。度量指标：任务完成时间、识别正确率、拿取-放回过程中的主观追踪连续性评分。
...
用户研究招募 12--16 名被试（平衡性别、年龄与 XR 经验水平），采用 \emph{Full} 与 \emph{ZOH} 的被试内配对设计，系统配置与任务顺序在参与者间平衡。每人完成系统介绍与训练（5 分钟）、正式任务、主观问卷（NASA-TLX、系统可用性量表、开放式反馈），总时长约 45--60 分钟。该比较评估完整连续锚定的任务层效应，不用于归因单个模块；\emph{No-StaticLock} 不进入用户研究。实验在典型室内光照条件下进行。

由于日常物体缺乏高精度真值，我们报告一组无需真值的客观代理指标：（1）\textbf{静止期抖动}------物体静止时锚点位置均方根（mm）；（2）\textbf{可靠性评分分布}------可见性、颜色与深度子分的时间序列统计；（3）\textbf{追踪丢失率}------生命周期进入丢失状态的时间占比；（4）\textbf{重获取频率}------系统或用户重新初始化对象追踪的次数；（5）\textbf{静止锚定占比}------输出处于静止锁定状态的时间比例；（6）\textbf{锁定期漂移}------静止锚定期间的累积位移（mm）。这些指标与主观评估共同描述应用中的对象泛化能力。

RQ1 的 \emph{Full} 与 \emph{No-StaticLock} 采用同帧配对结果，并按录制序列汇总。RQ2 的 \emph{Full} 与 \emph{ZOH} 在同一试次内同步配对；误差与最新观测时延按纳入帧汇总，未更新帧对占比按合格相邻帧对汇总，显示更新率按这些帧对的累计时长归一化。结果只作单会话描述性系统表征：渲染帧表示时间覆盖，不作为相互独立的统计样本，也不据此计算置信区间或显著性。用户实验将系统配置与任务或物体作为固定效应、参与者作为随机效应分析连续指标，Likert 条目采用有序混合模型，定性反馈采用主题编码分析。

\section{结果}


\subsection{RQ1：静态锚定质量}


RQ1 正式会话共采集 10,108 个 Unity 渲染帧，其中 9,057 帧位于静止观察与遮挡恢复标注范围内。参考有效性过滤后，静止观察纳入 4,306 帧，遮挡恢复纳入 4,374 帧；两种配置按同一渲染时刻形成 8,680 组有效帧配对（共 17,360 条配置记录），并关联 1,332 个去重视觉源帧标识。误差定义为渲染时刻锚点输出与同帧控制器平台参考位姿之间的差异。

\begin{table*}[tb]
  \centering
  \caption{RQ1 全部有效帧上的描述性结果。HP-RMS 依次按位置（mm）与旋转（°）报告，只在连续静止片段上解释为抖动；遮挡恢复任务包含生命周期切换，故不报告该项。箭头表示期望方向。}
  \label{tab:rq1-summary}
  \begin{tabularx}{\textwidth}{llr*{3}{>{\raggedleft\arraybackslash}X}}
\toprule
场景 & 配置 & 有效帧 & 平移 med / P95（mm）$\downarrow$ & 旋转 med / P95（$^\circ$）$\downarrow$ & 静止抖动 HP-RMS$\downarrow$ \\
\midrule
\multirow{2}{*}{静止观察} & \emph{Full} & \multirow{2}{*}{4,306} & 5.8 / 6.6 & 2.1 / 2.9 & 0.02 / 0.02 \\
 & \emph{No-StaticLock} & & 6.1 / 9.0 & 2.5 / 4.7 & 0.71 / 0.69 \\
\midrule
\multirow{2}{*}{遮挡恢复} & \emph{Full} & \multirow{2}{*}{4,374} & 5.6 / 6.7 & 2.2 / 4.6 & --- \\
 & \emph{No-StaticLock} & & 6.3 / 19.3 & 4.5 / 17.2 & --- \\
\bottomrule
  \end{tabularx}
\end{table*}
...
\begin{figure*}[t]
  ...
  \begin{minipage}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rq1/fig_rq1_position_timeline.pdf}
    \textbf{(a)} 位置 XYZ-t
  \end{minipage}%
  \hfill
  \begin{minipage}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rq1/fig_rq1_rotation_timeline.pdf}
    \textbf{(b)} 旋转 XYZ-t
  \end{minipage}%
  \caption{RQ1 静止观察的局部时间轨迹。窗口取首个 \emph{Full} 已进入静止锁定且两种配置与平台参考均有效的连续 180 个 Unity 渲染帧（550--729，共 2.99 s），选择规则不读取误差。(a) 相对窗口首帧平台参考位置的世界系 X/Y/Z 位移（mm）；(b) 相对同一参考姿态的世界系 SO(3) 对数向量（°）。黑线、蓝线与橙线分别表示平台参考、\emph{Full} 与 \emph{No-StaticLock}。全程统计使用表 \ref{tab:rq1-summary} 的完整序列。}
  \label{fig:rq1-static}
\end{figure*}


\textbf{精度与静止稳定性。} 静止观察中，两种配置的平移误差中位数接近（\emph{Full} 5.8 mm，\emph{No-StaticLock} 6.1 mm），说明静止锚定主要抑制波动而非改变稳态偏置。\emph{Full} 的位置与旋转 HP-RMS 均约 0.02（mm / °），\emph{No-StaticLock} 分别为 0.71 mm 与 0.69°，约为 \emph{Full} 的 33 倍与 42 倍。图 \ref{fig:rq1-static} 的同步三轴轨迹显示了冻结输出与逐帧波动之间的差异。

\textbf{遮挡鲁棒性。} 在遮挡恢复任务的完整录制片段中，\emph{Full} 的平移中位数 / P95 为 5.6 / 6.7 mm，\emph{No-StaticLock} 的 P95 升至 19.3 mm；旋转 P95 分别为 4.6° 与 17.2°。\emph{Full} 的生命周期状态占比为 Coasting 48\%、Searching 32\%、FrozenUncertain 15\% 与 Lost 5\%。由于日志未单独记录目标重新出现时刻，本实验不计算恢复时间。

\subsection{RQ2：动态锚定能力}


RQ2 正式会话共采集 12,918 个 Unity 渲染帧，其中 8,824 帧位于两个平移试次和一个旋转试次的标注范围内。自动活动提取保留 6,768 帧；再按参考新鲜度与速度范围筛选后，最终纳入 6,301 个有效帧（93.1\%）：平移 4,027 帧，旋转 2,274 帧。每个有效帧同步记录两种系统配置，形成 12,602 条配置记录，并关联 941 个去重视觉源帧标识。平移参考速度中位数 / P95 为 0.247 / 0.617 m/s，旋转为 94.1 / 165.0°/s。帧数描述时间覆盖和配对完整性；最高层独立单位仍只有一个会话，因此结果不作总体推断。

\begin{table*}[tb]
  \centering
  \caption{RQ2 全部有效帧上的描述性结果。显示更新率为更新帧对数除以累计时长，未更新帧对占比为显示位姿未发生可测变化的合格帧对比例；误差按任务主通道报告。两种配置的锚点输出覆盖率均为 100\%。箭头表示期望方向；本表不加粗单一“最佳”配置，因为连续性与瞬时精度构成权衡。}
  \label{tab:rq2-summary}
  \begin{tabularx}{\textwidth}{llr*{2}{>{\raggedleft\arraybackslash}X}}
\toprule
任务 & 配置 & 有效帧 & 显示更新率（Hz）$\uparrow$ / 未更新帧对占比（\%）$\downarrow$ & 误差 med / P95 $\downarrow$ \\
\midrule
\multirow{2}{*}{平移} & \emph{Full} & \multirow{2}{*}{4,027} & 59.1 Hz / 14.7\% & 81.5 / 208.3 mm \\
 & \emph{ZOH} & & 9.7 Hz / 86.0\% & 60.2 / 172.8 mm \\
\midrule
\multirow{2}{*}{旋转} & \emph{Full} & \multirow{2}{*}{2,274} & 66.3 Hz / 4.4\% & 29.4 / 62.7$^\circ$ \\
 & \emph{ZOH} & & 9.4 Hz / 86.5\% & 22.8 / 54.8$^\circ$ \\
\bottomrule
  \end{tabularx}
\end{table*}
...
\begin{figure*}[t]
  ...
  \begin{minipage}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rq2/fig_rq2_position_timeline.pdf}
    \textbf{(a)} 平移 XYZ-t
  \end{minipage}%
  \hfill
  \begin{minipage}[t]{0.495\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/rq2/fig_rq2_rotation_timeline.pdf}
    \textbf{(b)} 旋转 XYZ-t
  \end{minipage}%
  \caption{RQ2 中低速运动的局部时间轨迹，每幅图展示连续 120 个 Unity 渲染帧。(a) 平移试次 1 的帧 5314--5433（1.67 s），相对窗口首帧平台参考位置的世界系 X/Y/Z 位移（cm；窗口内参考速度中位数 0.185 m/s）；(b) 旋转试次 3 的帧 11126--11245（1.69 s），相对窗口首帧平台参考姿态的世界系 SO(3) 对数向量（°；窗口内参考角速度中位数 71.5°/s）。黑线、蓝线与橙线分别表示平台参考、\emph{Full} 与 \emph{ZOH}。总体统计使用全部 6,301 个有效帧，而非这些局部窗口。}
  \label{fig:rq2-timeline}
\end{figure*}


\textbf{连续性与精度。} 表 \ref{tab:rq2-summary} 汇总全部有效帧，图 \ref{fig:rq2-timeline} 展示局部轨迹形态。\emph{ZOH} 以 9.4--9.7 Hz 阶梯更新，未更新帧对占比约 86\%；\emph{Full} 在渲染帧间持续变化，显示更新率为 59.1--66.3 Hz，未更新帧对占比降至 4.4\%--14.7\%。这种连续输出伴随更高的渲染时刻误差：平移中位数由 60.2 增至 81.5 mm，旋转由 22.8° 增至 29.4°。因此，当前结果支持的是连续性与瞬时精度的权衡，而非单一配置在所有指标上占优。

\textbf{最新观测时延。} 两种配置共享同一视觉输入。平移与旋转任务中，该时延的中位数 / P95 分别为 223 / 389 ms 与 224 / 473 ms。本次录制未发生动态丢失或恢复事件，因此不评价恢复能力；连续性差异的任务层效应由 RQ3 的 \emph{Full} / \emph{ZOH} 配对设计检验。
...
\textbf{时间一致性的边界。} 帧对齐可避免将历史相机系观测与到达时刻相机位姿错误复合，但不能恢复目标物体在采集之后的当前位姿。动态场景仍受感知处理时间、观测更新率和平滑策略引入的插值延迟共同影响。RQ2 因此以渲染时刻误差描述应用实际获得的配准质量，不把帧对齐表述为对物体运动时延的补偿。

\subsection{局限与未来工作}


EgoAnchor 依赖目标物体的可用三维模型。缺少现成模型时，Image-to-3D 重建可补充模型来源，但重建误差会传递至锚定结果。严重遮挡、极弱纹理或超出有效深度量程也会降低感知质量与锚定连续性。定量轨采用与头显共享追踪系统的控制器平台位姿，因此无法观测共同的世界系漂移，也不能排除平台参考自身的预测、平滑与动态时延。文中误差均应理解为平台参考下的相对误差，而非外部光学系统给出的物理真值。RQ2 结果来自单次录制，只描述所设速度范围内的相对表现，不推断更高速度下的运行边界；旋转又只有一个试次，无法把误差归因于姿态噪声、观测更新率或策略滞后中的某一项。
...
本文提出 EgoAnchor，一套面向开放消费级混合现实的动态真实物体锚定系统。EgoAnchor 仅依赖头戴显示器双目图像与目标物体的可用三维模型，通过视觉感知后端与对象锚定运行时解耦的分层架构，将开放场景中低频、含噪且带传输时延的异步物体位姿流，转换为世界坐标系下高帧率、连续且可恢复的对象锚点。系统在采集时刻恢复相机坐标关系，并以运动估计、时序平滑、静止锚定与生命周期管理处理低频含噪观测，依据运动状态在静止与运动区制间切换行为。我们在 Meta Quest 3 上完成系统实现，并建立覆盖锚定精度、静止稳定性、最新观测时延、运动连续性与应用可用性的评估框架。该架构为开放消费级混合现实中的真实物体交互提供了可复现的实现基础。
