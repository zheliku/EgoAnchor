...
实验二复用实验一的候选、参考轨迹和渲染时间线。稳定配置 ID EgoAnchor 及采集时刻对齐、VCD、StaticLock 三个组件对照均使用 Kalman Linear/SLERP；时序合成消融改用 Kalman Predict-to-Now，但保留完整系统的其他组件。采集时刻对齐比较同一原始候选在 capture-time 与 arrival-time 世界复合下的误差；StaticLock 使用中心化静止波动；VCD 分开检验连续分数的风险判别性与冻结阈值的运行时接纳效果；时序合成比较完整 EgoAnchor 与 `w/o temporal synthesis'。
...
VCD score & event AURC / 全覆盖风险 & \multicolumn{3}{c}{tie-aware risk--coverage；待 v4 自动回填} \\
...
\begin{figure*}[t]
  ...
  \end{subfigure}
  \caption{实验二的组件归因与时序策略比较。图~(a)、(b) 分别比较采集时刻复合和 StaticLock；图~(c) 展示 VCD 分数诱导的 event 级风险--覆盖率，同分候选整组进入曲线；图~(d) 展示 Kalman Direct Predict-to-Now、Causal Prediction 与 Buffered Linear/SLERP。Causal 与 Buffered 共享关闭 StaticLock 的配置；Direct 单独用于时序合成消融。图和数值待 v4 自动回填。}
  \label{fig:exp2-final}
\end{figure*}

\textbf{组件归因。} 采集时刻对齐、StaticLock、VCD admission 和历史时序合成仍按单组件关闭进行配对分析。前三项分别报告同一候选的世界复合误差、中心化静止波动和遮挡尾部失效率；时序合成同时报告 fitted lag 与 lag-aligned residual。VCD score 另在每个最终有效的遮挡 event 内，以 capture-time raw pose 相对同 frame 平台参考的平均平移误差作为 selective risk，按分数从高到低诱导候选顺序；同分候选不可拆分，主指标为 event AURC 的 median [Q1, Q3]。VCD 分数是 $[0,1]$ 连续可靠性评分，不是位姿正确概率，也不是排序算法。结果均待 v4 自动回填。
...
所有误差均由 display pose 相对于同一 Quest 平台参考计算。静止头动场景同时报告绝对注册误差、片段中心化头动泄漏和帧间位置增量：中心化指标在片段内减去中位误差向量，用于隔离头动相关的世界坐标泄漏；帧间增量则量化相邻显示帧的静止抖动。采集时刻对齐另在 candidate 层面对同一原始观测的 capture-time 与 arrival-time 世界复合进行直接比较。遮挡场景同时报告 episode-level P95、超过 40~mm 的灾难性失效率和最大值，避免中位数掩盖稀疏重尾失效。VCD risk--coverage 不按冻结 admission 决策过滤候选；缺少 aligned raw 或同帧有效参考的行只进入排除审计，可评价候选若缺少有限 VCD 分数则硬失败。AURC 采用同分组整体加入后的右连续阶梯积分，并先按 event 计算，禁止跨 event 混池候选进行帧级推断。
