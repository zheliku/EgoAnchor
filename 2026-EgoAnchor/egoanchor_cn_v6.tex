...
% 自动发布的实验宏；缺失时不写入占位数字。
\IfFileExists{generated/exp1_numbers.tex}{\input{generated/exp1_numbers.tex}}{}
\IfFileExists{generated/exp2_numbers.tex}{\input{generated/exp2_numbers.tex}}{}
...
\textbf{持续平移与旋转。}
操作者分别执行中低速往复平移和交替轴向旋转，并控制速度不超过预设上限。该序列用于刻画连续运动下的显示误差、输出更新率、保持比例和时序滞后。

\textbf{遮挡与恢复。}
目标在正常追踪后经历多次部分遮挡和完全遮挡，并在重新可见后保持静止。实验记录遮挡开始、目标重新可见以及锚点恢复稳定的时刻，以测量遮挡期间的错误更新、输出保持、失效判定与重获取行为。

One Euro 参数、VCD 接纳阈值、时序合成参数、静止锚定参数和事件判定规则在系统实现完成时随配置固定。所有记录下来的实验 session 均为正式数据，采集后不再依据实验结果调参。

% 正式分析产物通过条件加载；缺失时不占用版面，也不写入占位数字。
\IfFileExists{generated/exp1_tables.tex}{%
  \begin{table}[t]
    \centering
    \caption{实验一四种系统配置的汇总结果。}
    \label{tab:exp1-generated-summary}
    \resizebox{\columnwidth}{!}{\input{generated/exp1_tables.tex}}
  \end{table}
}{}
\IfFileExists{figures/generated/exp1_system_summary.pdf}{%
  \begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/generated/exp1_system_summary.pdf}
    \caption{实验一四种系统配置的系统行为汇总。}
    \label{fig:exp1-generated-summary}
  \end{figure}
}{}
\IfFileExists{generated/exp1_numbers.tex}{%
  本批次包含 $\EAExpOneSessionCount$ 个正式 session，覆盖 $\EAExpOneScenarioCount$ 类预定义物理任务；系统优势以场景为条件，不能用跨场景混池的单一误差数字概括。在静止目标加主动头动场景，EgoAnchor 的平移误差中位数为 $\EAExpOneEgoAnchorStaticTranslationMedianMm$~mm、P95 为 $\EAExpOneEgoAnchorStaticTranslationPNinetyFiveMm$~mm，静止抖动 HP--RMS 为 $\EAExpOneEgoAnchorStaticJitterHpRmsMm$~mm，均优于 Arrival-Hold（P95 $\EAExpOneArrivalHoldStaticTranslationPNinetyFiveMm$~mm、抖动 $\EAExpOneArrivalHoldStaticJitterHpRmsMm$~mm），显示出显著更强的世界锁定稳定性。遮挡恢复场景同样体现该稳定性：EgoAnchor 平移 P95 为 $\EAExpOneEgoAnchorOcclusionTranslationPNinetyFiveMm$~mm，而 Arrival-Hold 为 $\EAExpOneArrivalHoldOcclusionTranslationPNinetyFiveMm$~mm。作为代价，在持续平移场景，延迟插值带来的运动阶段滞后使 EgoAnchor 平移中位数升至 $\EAExpOneEgoAnchorContTranslationTranslationMedianMm$~mm，高于 Arrival-Hold 的 $\EAExpOneArrivalHoldContTranslationTranslationMedianMm$~mm。这一“静止与遮挡阶段稳定、快速持续运动阶段有明确滞后代价”的结构是本系统的真实权衡，也是后续设计归因（实验二）与用户研究（实验三）的出发点，而非可以用单一排序掩盖的缺陷。

  到达时刻保持的最新观测年龄中位数/P95 为 $\EAExpOneArrivalHoldObservationAgePFiftyMs$/$\EAExpOneArrivalHoldObservationAgePNinetyFiveMs$~ms；三种采集时刻复合配置对应值均为 $\EAExpOneCaptureHoldObservationAgePFiftyMs$/$\EAExpOneCaptureHoldObservationAgePNinetyFiveMs$~ms。后者反映延迟插值主动使用历史控制点的时间语义，不应直接解读为感知链路处理时间更慢。
}{}
\IfFileExists{figures/generated/exp1_static_timeline.pdf}{%
  \begin{figure*}[t]
    \centering
    \includegraphics[width=0.32\textwidth]{figures/generated/exp1_static_timeline.pdf}
    \hfill
    \includegraphics[width=0.32\textwidth]{figures/generated/exp1_motion_timeline.pdf}
    \hfill
    \includegraphics[width=0.32\textwidth]{figures/generated/exp1_occlusion_recovery.pdf}
    \caption{实验一的静止头动、运动和遮挡恢复时间线。图中保留逐帧轨迹用于展示系统行为，统计推断仍在 trial/event 层完成。}
    \label{fig:exp1-generated-timelines}
  \end{figure*}
}{}
...
\textbf{采集时刻世界对齐。}
在静止目标与主动头动序列中比较完整 EgoAnchor 与关闭采集时刻世界对齐的变体。两种配置使用相同的 VCD 接纳、时序合成、静止锚定和生命周期管理，仅改变相机世界变换的查询时刻。分析平移与旋转误差，并考察误差随头部线速度和角速度的变化。

\textbf{VCD 观测接纳。}
在遮挡与低质量观测较多的序列上比较完整 EgoAnchor 与关闭 VCD 接纳的变体。两种配置共享相同的状态估计、时序合成和生命周期管理。分析风险--覆盖关系、固定候选接纳覆盖率下的 P95 位姿误差、明显错误候选数量以及下游输出可用率。VCD 在本文中被解释为锚点更新的运行时可靠性信号，而非位姿正确概率。

\textbf{时序合成。}
在起停六自由度序列中比较完整 EgoAnchor 与关闭时序合成的变体；两者保持采集时刻对齐、VCD 接纳、静止锚定和生命周期管理一致。该对比用于量化低频异步观测缺少逐帧时序合成时的连续性代价，以及显式合成在运动阶段的误差--滞后权衡。

\textbf{静止锚定。}
比较完整 EgoAnchor 与关闭静止锚定的变体。两种配置共享相同的候选接纳、状态估计、插值和生命周期策略。分析运动前后静止阶段的抖动、起动可见响应时间、停止后稳定时间以及运动阶段的位姿误差，从而量化静止稳定性收益及其转换代价。

\IfFileExists{generated/exp2_tables.tex}{%
  \begin{table*}[t]
    \centering
    \caption{实验二的组件消融汇总。}
    \label{tab:exp2-generated-summary}
    \resizebox{\textwidth}{!}{\input{generated/exp2_tables.tex}}
  \end{table*}
}{}
\IfFileExists{figures/generated/exp2_component_delta.pdf}{%
  \IfFileExists{figures/generated/exp2_vcd_risk_coverage.pdf}{%
    \begin{figure*}[t]
      \centering
      \includegraphics[width=0.48\textwidth]{figures/generated/exp2_component_delta.pdf}
      \hfill
      \includegraphics[width=0.48\textwidth]{figures/generated/exp2_vcd_risk_coverage.pdf}
      \caption{实验二的组件差值与 VCD risk--coverage 诊断。组件差值按消融配置减完整 EgoAnchor 计算。}
      \label{fig:exp2-generated-summary}
    \end{figure*}
  }{}
}{}
\IfFileExists{generated/exp2_numbers.tex}{%
  实验二的配对差值显示，关闭采集时刻对齐后静止头动平移误差中位数增加 $\EAExpTwoCaptureAlignmentTranslationMedianDeltaMm$~mm；关闭静止锚定后静止位置 HP--RMS 增加 $\EAExpTwoStaticLockPositionHpRmsDeltaMm$~mm。遮挡恢复场景中，关闭 VCD 的平移中位数差值为 $\EAExpTwoVCDTranslationMedianDeltaMm$~mm，当前批次的差异近似为零；完整系统 VCD 分数诱导的风险--覆盖 AURC 为 $\EAExpTwoAURC$~mm，因此该结果应视为本对象和本遮挡协议下的诊断，而不是 VCD 泛化有效性的证明。时序合成差值为 $\EAExpTwoTemporalVisibleResponseDeltaMs$~ms，但对应起停事件只有单个可配对条目，本文将其作为描述性证据，不作总体性结论。
}{}
\clearpage

\subsection{评价指标与汇总}

所有序列均报告相对于平台参考的平移误差和旋转误差，包括中位数、四分位距与 P95。为刻画稳定性与时效性之间的权衡，我们报告\emph{最新观测时滞}，定义为渲染时刻与当前状态所纳入的最新视觉观测采集时刻之差；对于采用延迟插值的配置，进一步报告\emph{输出时间滞后}，定义为渲染时刻与当前显示位姿所代表的目标时刻之差。前者反映运行时所利用视觉信息的时效性，后者反映时序合成主动引入的显示延迟。

静止序列报告位置和旋转抖动；起停序列报告平台参考开始运动至锚点产生可见响应的时间，以及物体停止后锚点重新进入稳定状态所需的时间；持续运动序列报告输出更新率、保持比例和运动阶段误差；遮挡恢复序列报告输出可用率、位姿更新跳变的 P95、恢复成功率以及目标重新可见至稳定恢复的时间。视觉感知频率、应用渲染频率和图像采集至候选抵达的感知处理时延作为系统运行特征一并报告。
