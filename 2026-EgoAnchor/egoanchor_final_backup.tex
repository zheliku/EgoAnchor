...
\teaser{
  ...
  \caption{EgoAnchor 系统概览与能力定位。\textbf{(上)} 现有方案的能力矩阵：没有任何系统同时满足免主动深度、任意物体、允许运动、低成本模型获取与开放硬件这五个维度。\textbf{(中)} EgoAnchor 端到端架构：Quest~3 双目采集 → 外部追踪服务（语义分割/双目深度/6DoF 估计）→ 四层协同锚定运行时（时间对齐/质量门控/时序平滑/生命周期管理）。\textbf{(下)} 对比效果：传统到达时刻映射产生打滑，EgoAnchor 实现稳定贴合。占位图：建议最终展示 (a) 能力对比表、(b) 系统架构图、(c) 打滑 vs 贴合对比。}
  \label{fig:teaser}
}

\abstract{%
透视混合现实中虚拟内容与物理对象的稳定锚定是交互可用性的基础，但现有方案存在能力覆盖空白。平台方案或依赖专有深度传感器（HoloLens、Vision~Pro），或局限于静态物体（Vuforia），或仅支持预定义类别（Meta）。本文提出 \textbf{EgoAnchor}，首个同时实现五维能力的系统：无需训练、纯双目感知、任意刚性日常物体、动态追踪、开放消费级硬件。核心洞察是：世界一致的锚定需要时间对齐、质量门控、时序平滑与生命周期管理的系统协同，而非仅依赖单帧位姿精度。EgoAnchor 构建四层架构：(1)~帧对齐锚定通过回查采集时刻相机位姿解耦感知延迟与头部运动；(2)~可靠性门控以多维评分过滤异常观测；(3)~卡尔曼状态估计与静止锁定抑制抖动；(4)~生命周期管理实现遮挡后快速重获取。系统以 5--12\,fps 感知输入在消费级 GPU 上产生 60\,fps 世界一致锚点。在 Quest~3 上的评估表明其锚定质量与平台级方案相当，为开放 MR 物理交互提供可行路径。代码将开源。}
...
透视式混合现实（passthrough mixed reality, PMR）正在把虚拟信息直接叠加到用户所处的真实环境之上。在虚拟标签附着、实物操作指导、近距交互辅助等典型场景中，虚拟内容必须稳定地\emph{贴}在对应的物理实体上：用户转头、走近、伸手操作时，叠加图层应当始终与真实物体保持一致，既不漂移，也不抖动。实现这种稳定锚定需要系统同时满足多个维度的能力要求，然而现有方案各有取舍，没有任何一个能够覆盖日常通用交互的全部需求。


带主动深度的方案能够提供高质量的三维感知，但依赖专有硬件。HoloLens~2 上的 Azure Object Anchors 依赖空间映射的主动深度，但已于 2023 年底停止服务~\cite{azureObjectAnchors}。Apple Vision Pro 在 visionOS~27 中首次将预训练对象追踪扩展到手持与运动物体~\cite{appleVisionOS27ObjectTracking}，这是重要进展，但它依赖带 LiDAR 的封闭设备、需要数小时的 Create~ML 离线训练，且高帧率路径据报告也仅约 30\,Hz。仅凭相机与三维模型工作的 Vuforia Model Targets 通过边缘匹配实现免标记追踪，但被明确定位为\emph{静态}物体，物体移动后难以保持~\cite{vuforiaModelTargets}。当平台支持\emph{运动}物体时，覆盖范围又收缩到极有限的固定类别：Meta Quest 的动态物体追踪实际仅稳定支持键盘等少数预定义类，且官方说明其更新率不适合实时跟随运动物体~\cite{metaDynamicObjectTracker,metaSceneOverview}。跨越\textbf{免主动深度、任意日常物体、允许运动、模型可低成本获取、运行于开放消费级硬件}这五个维度的组合，至今仍是空白。

为填补这一空白，我们提出 \textbf{EgoAnchor}，一套仅依赖双目透视图像与物体三维模型的端到端真实物体锚定系统。系统的物体模型可由日常手机照片经图像到三维生成获得~\cite{trellis2024,appleObjectCapture}，因而原则上可覆盖鼠标、耳机、手柄等任意刚性日常物体；整套感知-锚定回路运行于消费级 GPU 之上，无需针对每个物体进行训练。EgoAnchor 的核心并非提出新的位姿估计算法，而是回答一个系统问题：\textbf{如何把低频（5--12\,fps）、含噪、会间歇失效的异步外部感知流，转化为高频（60\,fps）、世界一致、稳定且可恢复的真实物体锚点？}

这一问题的核心挑战在于\textbf{时间异步}。为获得可靠的 6DoF 估计，系统通常把神经网络推理放在头显之外的算力上；受推理耗时所限，外部感知是低频、异步的，而头显渲染必须维持 90\,fps 以上。当外部追踪包到达时，若直接将其携带的相机坐标系位姿与\emph{到达时刻}的最新头显位姿相乘以映射到世界空间，头部在采集到回传这段时延内的任何运动都会被错误地耦合进来，在视觉上表现为明显的「打滑」。此外，外部视觉追踪在运动模糊、光照变化或局部遮挡下不可避免地会输出噪声乃至位姿跃变，未经处理直接驱动虚拟内容会产生剧烈抖动。这些系统性挑战不会因单帧位姿精度的提升而自然消失。

EgoAnchor 通过\textbf{四层协同架构}解决上述挑战：

\noindent\textbf{(1)~时间对齐层。} 通过\emph{帧对齐锚定（frame-aligned anchoring）}机制，系统为每帧透视图像绑定采集时刻的 \texttt{frame\_id}，当外部服务返回对应该帧的 6DoF 观测时，按 \texttt{frame\_id} 回查\emph{采集时刻}缓存的头显位姿进行世界空间映射，而非与\emph{到达时刻}的最新头显位姿相乘。这一设计可以理解为渲染管线中 late latching / 异步时间扭曲原理~\cite{vanwaveren2016atw,mark1997postrendering}在异步感知融合层的对偶：时间扭曲把已渲染画面对齐到\emph{当前}最新头显位姿，帧对齐锚定把外部感知结果对齐回其\emph{采集时刻}的头显位姿，从而以精确而非预测的方式消除头动诱发的打滑。

\noindent\textbf{(2)~质量门控层。} 以多维可靠性评分（LAB 颜色空间的重投影误差、掩膜内渲染深度有效性）作为门控，拦截低质量观测，防止异常位姿污染锚点。

\noindent\textbf{(3)~时序平滑层。} 采用恒速卡尔曼状态估计对物体的世界坐标位置、速度与旋转进行自适应平滑；当判定物体处于静止窗口时，启动\emph{静止锁定（static lock）}以死区、累积偏差与漂移约束完全抑制残余微抖。

\noindent\textbf{(4)~生命周期层。} 在遮挡或追踪失效时进行有界保持，并在可靠性持续缺失时触发向后端的重获取请求，实现快速复位与自愈。

这四层协同设计将异步外部感知流转化为真正可用的混合现实锚点。我们强调，各层模块各自并不追求算法新颖性，其价值在于被组织为一套针对混合现实异步感知场景的完整解决方案。

本文贡献如下：
\begin{enumerate}[leftmargin=1.4em]
  \item \textbf{首个五维能力空白的系统。} EgoAnchor 是首个同时满足免主动深度、任意日常物体、允许运动、模型可低成本获取、运行于开放消费级硬件这五个维度的端到端真实物体锚定系统。
  \item \textbf{针对异步MR感知的四层协同架构。} 我们设计并实现了时间对齐、质量门控、时序平滑、生命周期管理四层协同的锚定运行时，将低频、含噪、会间歇失效的外部感知流转化为高频、世界一致、稳定可恢复的锚点。
  \item \textbf{以锚点为中心的评估协议。} 我们建立了从世界锚定误差、静态抖动、头动打滑、端到端时延到恢复时间的评估协议，将评价重心从单帧相机坐标系位姿精度转向应用层锚点质量。
  \item \textbf{开源实现与可复现。} 系统在 Quest~3 与消费级 GPU 上实现，实验表明其达到与平台级方案相当的锚定质量；完整代码将开源以促进后续研究。
\end{enumerate}
...
\noindent\textbf{四层锚定运行时（Unity）。} 订阅后端位姿流，依次经过：(1)~时间对齐层（帧对齐锚定），(2)~质量门控层（可靠性评分过滤），(3)~时序平滑层（卡尔曼状态估计与静止锁定），(4)~生命周期层（保持/重获取决策）。最终输出 60\,fps 的世界坐标锚点驱动应用层虚拟内容。
