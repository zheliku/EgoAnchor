...
\abstract{%
以真实物体为中心的混合现实应用需要能够随物体运动持续更新的对象级空间参考。然而，异步视觉后端输出的六自由度位姿对应历史图像采集时刻，候选质量随遮挡与几何条件波动，且低频、含噪的位姿流不能直接满足逐帧渲染对稳定性、连续性与生命周期管理的要求。本文提出 EgoAnchor，一套面向消费级混合现实的端到端动态真实物体锚定系统。系统仅依赖头戴显示器双目图像和目标物体的可用三维模型，无需物理标记、物体侧追踪硬件或逐物体训练。EgoAnchor 通过采集时刻世界对齐恢复异步观测的时间语义，以可视度门控的颜色--深度一致性评分限制有害更新，并通过 Kalman--Hermite 时序合成、显式静止锚定与生命周期管理生成应用可持续消费的世界系对象锚点。我们在 Meta Quest 3 上实现系统，并在同一视觉候选流、渲染时间线和平台参考下进行配对系统表征。静止头动时，EgoAnchor 的片段内平移 P95 为 3.679~mm，而 Arrival-Hold 为 22.237~mm；持续平移时，其有效时延为 325~ms，但 lag-aligned residual 为 5.697~mm，低于 Arrival-Hold 的 10.568~mm；遮挡窗内，其平移 P95 为 1.822~mm，而 One-Euro Anchor 为 12.999~mm。结果表明，EgoAnchor 明显改善静止附着与失效控制，同时以可测量的运动时延换取连续轨迹质量。上述数值描述同一设备与时间线下相对于平台参考的系统行为，并非外部物理真值。
}
...
数据由五个场景专用连续 session 构成，共记录 29,316 个唯一渲染时刻与 6,838 个视觉位姿候选。相同数据流在 8 种运行时配置下同步回放后形成 234,528 条 configuration--frame records；该记录数反映配对重放规模，并不代表 234,528 次独立采集。相邻渲染帧具有强时间相关性，因此帧仅用于重建连续轨迹，不作为独立统计样本。各指标首先在预定义的重复动作片段或遮挡过程内计算，再汇总片段间的 median [IQR]。由于每个场景仅有一条连续 trial，本文不进行帧级显著性检验，也不将片段间波动解释为跨环境或跨操作者的总体推断。
...
实验一围绕四项应用可感知属性组织：\emph{world consistency} 衡量头动是否被错误写入静止物体的世界位置；\emph{rest stability} 衡量静止锚点的显示抖动；\emph{dynamic fidelity} 将持续运动中的轨迹时延与时延对齐后的残差作为一组不可拆分的权衡；\emph{failure containment} 衡量遮挡和坏观测是否破坏已建立锚点。表~\ref{tab:exp1-final} 仅突出各场景的主指标；动态平移以 ``lag / aligned residual'' 成对报告，起停响应则单独标为稳定优先策略的转换代价。图~\ref{fig:exp1-final} 叠加全部重复动作实例与 median--IQR 汇总。
...
EgoAnchor 将静止头动场景的片段内平移 P95 从 Arrival-Hold 的 22.237~mm 降至 3.679~mm（降低 83.5\%），相对 One-Euro Anchor 的 8.273~mm 仍降低 55.5\%。Capture-Hold 已将误差降至 8.573~mm，说明恢复采集时刻的相机世界变换可移除大部分由头动造成的错误复合；完整 EgoAnchor 进一步通过候选接纳和 StaticLock 抑制剩余波动。EgoAnchor 的 HP--RMS 为 0.030~mm，但该量描述静止锁定后的\emph{显示锚点抖动}，不等同于 0.03~mm 的物理追踪精度。

\textbf{持续平移中的时延--轨迹质量权衡。}
零时延参考下的动态误差会混合时间滞后与空间轨迹偏差，因此我们将拟合时延和 lag-aligned residual 作为同一指标组。相比 Arrival-Hold，EgoAnchor 的有效时延由 182.5~ms 增至 325~ms，但 residual 从 10.568~mm 降至 5.697~mm（降低 46.1\%）。相比 One-Euro Anchor，EgoAnchor 同时减少 62.5~ms 的拟合时延，并将 residual 降低 46.9\%。One-Euro 的较大拟合时延并非显式设置的渲染缓冲，而是低通平滑作用于已经滞后且不规则采样的历史观测后产生的相位滞后；它仍是噪声输入平滑的合理工程基线，但不负责候选接纳、时间语义恢复或生命周期管理。

该收益没有一致延伸至旋转：EgoAnchor 的有效角时延 / residual 为 350~ms / 3.620$^\circ$，而 Capture-Hold 为 265~ms / 2.909$^\circ$。因此我们不主张当前时序模型在所有运动通道上占优；旋转速度估计和对数空间插值仍是明确的改进方向。

\textbf{遮挡期间的失效控制。}
遮挡窗内，EgoAnchor 的平移 P95 为 1.822~mm，相比 Arrival-Hold 降低 94.8\%，相比 One-Euro Anchor 降低 86.0\%。该结果表明系统的主要价值不是在不可见期间推断未知物体运动，而是阻止低质量候选破坏已有空间参考。四种方法的持久恢复时间在本批次均为 207.577~ms，主要受共享候选重新到达时间约束，因而不作为方法排名指标。

\textbf{稳定优先策略的转换代价。}
起停 6DoF 中，EgoAnchor 的 start-transition response 为 751.053~ms，明显长于 Arrival-Hold 的 126.344~ms。该量不是原始感知时延：它从参考物体开始运动计时，包含 StaticLock 的解锁证据积累、视觉候选更新和延迟合成时间轴，刻画的是锚点从静止稳定模式切换到可见动态跟随的总代价。该结果限定了系统当前适用边界，并由实验三进一步检验其对抓取和放置任务的影响。

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.99\textwidth]{../figures/generated/exp1_final_v2.pdf}
  \caption{实验一的核心系统行为。小标记表示重复动作片段或遮挡过程，大标记与误差棒表示 median--IQR。左：主动头动下的静止物体世界一致性；中：持续平移的 fitted-lag--aligned-residual 联合权衡，越靠左下越好；右：遮挡期间坏观测对已有锚点的破坏程度。左、右面板从零开始；权衡图按数据范围缩放。}
  \label{fig:exp1-final}
\end{figure*}

\subsection{实验二：组件归因}

实验二复用实验一的同一候选流和参考轨迹，并在完整 EgoAnchor 上关闭单一机制。为避免“哪一列最小”替代机制解释，每个组件只使用与其设计目标对应的主指标；护栏只用于限定代价，不参与加粗排名。表~\ref{tab:exp2-final} 以完整系统为锚点报告关闭组件后的片段级变化，图~\ref{fig:exp2-final} 将三个目标组件效应合并，并以独立面板呈现时序合成的时延--残差权衡。
...
\begin{figure*}[t]
  ...
  \caption{实验二的组件归因被合并为两个逻辑面板。左侧三个紧凑配对图分别显示采集时刻对齐、StaticLock 与 VCD 的目标效应；每条细线连接同一重复动作片段或遮挡过程的 Full--Disabled 结果，粗线表示中位数。右侧单独显示时序合成的 fitted-lag--aligned-residual 权衡。左侧绝对误差坐标从零开始，右侧权衡图按数据范围缩放。}
  \label{fig:exp2-final}
\end{figure*}

关闭采集时刻对齐后，所有重复头动片段均向不利方向变化，平移 P95 的配对中位增量为 5.585~mm。这直接支持历史相机系观测必须按采集时刻设备位姿进入世界坐标系。关闭 StaticLock 后，HP--RMS 增加 1.592~mm，且绝对平移误差中位数增加 1.197~mm，说明静止冻结不仅抑制高频抖动，也改善了显示锚点中心的稳定性。

关闭 VCD 后，遮挡窗平移 P95 增加 21.857~mm，而共享候选约束下的持久恢复时间不变。候选级诊断显示，在 574 个 risk-eligible candidate 上，按 VCD 排序的 mean-risk AURC 为 2.384~mm，随机顺序参考为 4.081~mm。该结果只支持本对象与本遮挡协议中的风险排序能力，不将 VCD 分数解释为跨对象校准的正确概率。

关闭时序合成后，持续平移的拟合时延由 325~ms 降至 255~ms，但 lag-aligned residual 由 5.697~mm 增至 11.012~mm（增加 93.3\%）。因此，该组件的作用不是最小化时延，而是为低频异步候选流定义连续的逐帧显示轨迹；更低的无合成时延不能脱离其更大的轨迹残差单独解释为优势。起停序列中 P95 与 P99 跳变差值方向相反，也进一步说明单一跳变分位数不足以概括该组件。
\FloatBarrier

\subsection{评价指标与汇总契约}

所有误差均由 display pose 相对于同一 Quest 平台参考计算。静止场景报告绝对误差、漂移与高通 RMS；起停场景报告运动窗误差、跳变、可见响应和重新稳定行为；遮挡场景在遮挡窗和重新可见后的恢复窗分别计算误差与状态指标。持续运动的\emph{有效时延}在每个连续动作片段内通过冻结 lag 网格搜索得到：平移通道最小化 $\mathrm{display}(t)$ 与 $\mathrm{reference}(t-\mathrm{lag})$ 的 RMSE，旋转通道最小化对应角残差，并满足最小连续区间和重叠长度。该量是相对于平台参考轨迹的描述性拟合时延，不等于网络到达时延，也不等于运行时设定的插值缓冲。

本文以 median [IQR] 和同一动作片段内的配对差值为主，不进行渲染帧级显著性推断。候选级 VCD risk--coverage 独立于片段级系统比较，仅用于检验评分能否优先保留低风险候选。所有参数、动作片段边界和指标契约在正式数据分析前冻结；完整片段级表、图源数据和流式解析脚本随分析包提供。
