...
namespace EgoAnchor.Runtime
{
    ...
    public sealed class PoseToAnchorRuntime : MonoBehaviour
    {
        ...
        /// <summary>最近一次平滑 Kalman 外推实际使用的预测时域，单位毫秒。</summary>
        private double latestPredictionHorizonMs = double.NaN;

        /// <summary>最近一次平滑 Kalman 外推实际施加的位置校正残差，单位米。</summary>
        private float latestCorrectionPositionResidualMeters = float.NaN;

        /// <summary>最近一次平滑 Kalman 外推实际施加的旋转校正残差，单位度。</summary>
        private float latestCorrectionRotationResidualDegrees = float.NaN;

        /// <summary>平滑 Kalman 外推因异常输入放弃连续性的累计次数。</summary>
        ...
        /// <summary>最近一次平滑 Kalman 外推实际使用的预测时域，单位毫秒；其他策略为 NaN。</summary>
        public double LatestPredictionHorizonMs => latestPredictionHorizonMs;

        /// <summary>最近一次平滑 Kalman 外推实际施加的位置校正残差，单位米；其他策略为 NaN。</summary>
        public float LatestCorrectionPositionResidualMeters => latestCorrectionPositionResidualMeters;

        /// <summary>最近一次平滑 Kalman 外推实际施加的旋转校正残差，单位度；其他策略为 NaN。</summary>
