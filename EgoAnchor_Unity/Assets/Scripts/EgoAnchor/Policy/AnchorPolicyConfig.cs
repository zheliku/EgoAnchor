...
namespace EgoAnchor.Policy
{
    ...
    public sealed class AnchorPolicyConfig
    {
        ...
        [Tooltip("静止模式下单帧测量与预测的位置差超过该值时立即退出静止。防止强平滑把缓慢真实移动当噪声吸收。")]
        [Min(0.001f)] public float staticExitDisplacement = 0.02f;

        /// <summary>立即退出静止的测量旋转差阈值，单位度。</summary>
        [Tooltip("静止模式下单帧测量与预测的旋转差超过该值时立即退出静止。")]
        [Min(0.1f)] public float staticExitRotationDeg = 3f;

        /// <summary>超过该时长没有被接受的测量才进入 Coasting，单位秒。</summary>
        [Header("时序与续航")]
        [Tooltip("超过该时长没有被接受的测量才进入 Coasting，单位秒。应覆盖正常 pose 消息间隔（10Hz 流约 0.1s），避免消息间隙被误判为丢失。")]
        [Min(0.02f)] public float coastGraceSeconds = 0.18f;
